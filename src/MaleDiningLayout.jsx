...
export default function MaleDiningLayout({ occupied, selected, onSelect, onClose }) {
  ...
  const renderCell = (num, type) => {
    ...
    let bg = isOcc ? '#ffebee' : (isSel ? '#007bff' : (type === 'Floor' ? '#fff9c4' : 'white'));
    let color = isOcc ? '#aaa' : (isSel ? 'white' : '#333');
    return (
      <button key={num} onClick={() => !isOcc && onSelect(sNum, type)} disabled={isOcc} style={{ width: '100%', height: '35px', margin: '2px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', background: bg, border: '1px solid #ccc', borderRadius: '4px', cursor: isOcc ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '13px', color }}>{num}</button>
    );
  };
