...
export default function ParticipantList({ courses, refreshCourses }) {
  ...
  // ✅ UPDATED DEFAULTS: Female (A-H = 8), Male (A-J = 10)
  ...
  // ✅ GRID RENDERER (Takes full array of ordered columns)
  const renderGrid = (map, orderedCols, rows, gender) => { 
      let g=[]; 
      for(let r=rows; r>=1; r--) { 
          let cells = orderedCols.map((c, idx) => {
             if(c === 'GAP') return <div key={`gap-${r}`} style={{width:'30px', writingMode:'vertical-rl', fontSize:'9px', color:'#ccc', display:'flex', alignItems:'center', justifyContent:'center'}}>{r===Math.ceil(rows/2) && "PATHWAY"}</div>;
  ...
  if (viewMode === 'seating') { 
      ...
      // ✅ PREPARE COLUMNS FOR PRINT VIEW (Reversed Female)
      const mReg = getAlphabetRange(0, seatingConfig.mCols); // A-J
      const mSpec = generateChowkyLabels(seatingConfig.mCols, seatingConfig.mChowky); // K-L
      const mOrdered = [...mReg, 'GAP', ...mSpec];

      const fReg = getAlphabetRange(0, seatingConfig.fCols); // A-H
      const fSpec = generateChowkyLabels(seatingConfig.fCols, seatingConfig.fChowky); // I-J
      const fOrdered = [...fSpec.reverse(), 'GAP', ...fReg.reverse()]; // Chowky - Gap - H...A
          ...
          <div id={id} style={{width:'100%', maxWidth:'1500px', margin:'0 auto'}}> 
              <div className="no-print" style={{textAlign:'right', marginBottom:'10px', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#f8f9fa', padding:'10px', borderRadius:'8px'}}> 
                  <div style={{display:'flex', gap:'5px', alignItems:'center'}}> 
                      ...
                      <span style={{fontSize:'12px', fontWeight:'bold'}}>Std Cols:</span>
  ...
  // ✅ PASS ORDERED ARRAYS TO VISUAL HALL
