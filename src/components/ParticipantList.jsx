...
export default function ParticipantList({ courses, refreshCourses, userRole }) {
  ...
  // ✅ AUTO-SIZE GRID (REQUIREMENT AWARE)
  const handleAutoScaleGrid = () => {
      // Get Requirements from current live stats
      const { m, f } = seatingStats; // { chowky: X, std: Y }

      // 1. Determine Row Count (Hall Depth)
      // Heuristic: Halls usually have 8-12 rows. 
      // We take the side with MORE total students to decide the depth.
      const maxStudents = Math.max(m.chowky + m.std, f.chowky + f.std);
      let calculatedRows = Math.ceil(maxStudents / 8); // Assume roughly 8 cols wide
      if (calculatedRows < 8) calculatedRows = 8;      // Min floor
      if (calculatedRows > 14) calculatedRows = 14;    // Max ceiling (hall likely gets wider, not infinitely deeper)

      // 2. Calculate Columns based on Specific Needs
      // Formula: Need / Rows -> Round Up
      const calcCols = (need) => Math.ceil(need / calculatedRows);

      const mChowkyCols = calcCols(m.chowky);
      const mStdCols = calcCols(m.std) + 1; // +1 Buffer for growth

      const fChowkyCols = calcCols(f.chowky);
      const fStdCols = calcCols(f.std) + 1; // +1 Buffer for growth
      ...
      alert(`✨ Grid Auto-Sized based on Requirements!\n\nRows: ${calculatedRows}\nMale: ${mChowkyCols} Chowky Cols + ${mStdCols} Std Cols\nFemale: ${fChowkyCols} Chowky Cols + ${fStdCols} Std Cols`);
  };
  ...
  return (
    <div style={styles.card}>
      ...
      {/* ✅ FIXED: Professional Box Grid for ID Card */}
      {printReceiptData && <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:9999}}><div style={{background:'white', padding:'20px', borderRadius:'10px', width:'350px'}}><button onClick={()=>setPrintReceiptData(null)} style={{float:'right', background:'red', color:'white', border:'none', borderRadius:'50%', width:'30px', height:'30px', cursor:'pointer'}}>X</button><div id="receipt-print-area" style={{padding:'15px', border:'4px solid black', fontFamily:'Helvetica, Arial, sans-serif', color:'black', margin:'0 auto', maxWidth:'300px'}}><div style={{textAlign:'center', fontWeight:'bold', marginBottom:'10px'}}><div style={{fontSize:'18px', textTransform:'uppercase'}}>VIPASSANA</div><div style={{fontSize:'11px'}}>International Meditation Center</div><div style={{fontSize:'13px', marginTop:'2px'}}>Dhamma Nagajjuna 2</div></div><div style={{borderBottom:'2px solid black', margin:'5px 0'}}></div><div style={{fontSize:'12px', marginBottom:'10px', lineHeight:'1.4'}}><div><strong>Course:</strong> {printReceiptData.courseName}</div><div><strong>Teacher:</strong> {printReceiptData.teacherName}</div><div><strong>Dates:</strong> {printReceiptData.from} to {printReceiptData.to}</div></div><div style={{borderBottom:'2px solid black', margin:'5px 0'}}></div><div style={{fontSize:'16px', fontWeight:'bold', margin:'10px 0', textTransform:'uppercase'}}><div>{printReceiptData.studentName}</div><div style={{fontSize:'13px', fontWeight:'normal'}}>ID: {printReceiptData.confNo}</div></div><table style={{width:'100%', fontSize:'14px', border:'2px solid black', borderCollapse:'collapse', marginTop:'10px'}}><tbody><tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Room</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center', fontSize:'16px'}}>{printReceiptData.roomNo}</td></tr><tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Dining</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center', fontSize:'16px'}}>{printReceiptData.seatNo}</td></tr><tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Lockers</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center'}}>{printReceiptData.lockers}</td></tr><tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Lang</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center'}}>{printReceiptData.language}</td></tr>{printReceiptData.pagoda && <tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Pagoda</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center'}}>{printReceiptData.pagoda}</td></tr>}{printReceiptData.special && <tr><td style={{border:'1px solid black', padding:'6px', background:'#f0f0f0'}}>Special</td><td style={{border:'1px solid black', padding:'6px', fontWeight:'bold', textAlign:'center'}}>{printReceiptData.special}</td></tr>}</tbody></table><div style={{textAlign:'center', fontSize:'10px', fontStyle:'italic', marginTop:'10px'}}>*** Student Copy ***</div></div><div className="no-print" style={{marginTop:'20px', display:'flex', gap:'10px'}}><button onClick={() => window.print()} style={{flex:1, padding:'12px', background:'#007bff', color:'white', border:'none', borderRadius:'6px', fontWeight:'bold'}}>PRINT ID CARD</button></div></div><style>{`@media print { body * { visibility: hidden; } #receipt-print-area, #receipt-print-area * { visibility: visible; } #receipt-print-area { position: absolute; left: 0; top: 0; width: 100%; margin: 0; padding: 0; border: none; } @page { size: 80mm auto; margin: 0; } }`}</style></div>}
