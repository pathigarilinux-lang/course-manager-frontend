...
export default function ParticipantList({ courses, refreshCourses, userRole }) {
  ...
  // ✅ AUTO-GENERATE GRID SIZE
  const handleAutoScaleGrid = () => {
      const males = participants.filter(p => p.status === 'Attending' && (p.gender||'').toLowerCase().startsWith('m'));
      const females = participants.filter(p => p.status === 'Attending' && (p.gender||'').toLowerCase().startsWith('f'));
      
      const mCount = males.length + 5; // buffer
      const fCount = females.length + 5; 

      // Simple heuristic: Aim for roughly square or slight landscape
      // E.g. If 50 students, maybe 5 cols x 10 rows
      const calcDims = (count, isChowky) => {
          if (count === 0) return { cols: 5, rows: 10 }; 
          const cols = Math.ceil(Math.sqrt(count / 1.5)); // Aspect ratio tweak
          const rows = Math.ceil(count / cols);
          return { cols, rows };
      };

      const mDims = calcDims(mCount);
      const fDims = calcDims(fCount);

      // Normalize Rows (use max row height so hall looks symmetric)
      const maxRows = Math.max(mDims.rows, fDims.rows, 10);

      setSeatingConfig({
          ...seatingConfig,
          mCols: mDims.cols, mRows: maxRows,
          fCols: fDims.cols, fRows: maxRows
      });
      alert(`✨ Grid Auto-Sized!\nMale: ${mDims.cols}x${maxRows}\nFemale: ${fDims.cols}x${maxRows}`);
  };
