...
export default function FemaleBlockLayout({ rooms, occupancy, onRoomClick }) {
    ...
    const getOthers = () => {
        const mainKeys = new Set([
            ...[201, 202, 203, 204, 205, 206, 207, 208, 209, 210, 211, 212], // A
            ...[213, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223, 224], // B
            ...[225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242], // C
            ...[243, 244, 245, 246, 247, 248] // D
        ]);
        
        return Object.values(allRooms).filter(g => {
            // Check if it's in main numeric keys OR is an FRC key
    ...
    const getBlockStats = (roomList) => {
        ...
        roomList.forEach(num => {
            ...
            if (group) { totalBeds += group.beds.length; occupiedBeds += group.beds.filter(b => b.occupant).length; }
        });
    ...
    const RoomBox = ({ num, groupData = null }) => {
        ...
        return (
            <div title={isMedical ? "Reserved for Medical" : ""} style={{border: boxBorder, borderRadius: '4px', overflow:'hidden', background: isMedical ? '#fff8e1' : 'white', height:'100%', display:'flex', flexDirection:'column', minHeight:'60px'}}>
    ...
    return (
        <div style={{display:'flex', flexDirection:'column', gap:'20px', overflowX:'auto'}}>
            ...
            {/* ✅ RENDERS ANY MANUAL/TEMP ROOM HERE */}
            {getOthers().length > 0 && (
