...
export default function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  ...
  // ✅ UPDATED: Fetch Participants AND Global Conflicts when Course Changes
  useEffect(() => { 
      if (formData.courseId) {
          // 1. Fetch Students of THIS course
          fetch(`${API_URL}/courses/${formData.courseId}/participants`).then(res => res.json()).then(setParticipants);
          
          // 2. Fetch Occupied Seats from OTHER Overlapping Courses
  ...
  // ✅ MERGE LOCAL AND GLOBAL OCCUPANCY
  const usedDining = new Set(globalOccupied.dining); // Start with global blocks
  const usedPagoda = new Set(globalOccupied.pagoda); // Start with global blocks
  
  const occupiedDiningData = {}; 
  const occupiedPagodaData = {}; 
  ...
  participants.forEach(p => {
      ...
      // Only block seats if gender matches (Dining is usually gender segregated anyway)
      if ((isMale && pGender.startsWith('m')) || (isFemale && pGender.startsWith('f'))) {
          ...
          if (p.dining_seat_no) {
              ...
              usedDining.add(seat);
              occupiedDiningData[seat] = cat; // Add visual badge
          }
          if (p.pagoda_cell_no) {
              const cell = cleanNum(p.pagoda_cell_no);
              usedPagoda.add(cell);
              occupiedPagodaData[cell] = cat; // Add visual badge
          }
      }
  });
                  ...
                  <div style={{background:'#f8f9fa', padding:'15px', borderRadius:'10px', marginBottom:'15px', border:'1px solid #eee'}}>
                      <div style={{display:'grid', gridTemplateColumns:'1fr 2fr', gap:'15px', alignItems:'end'}}>
                          <div>
                              ...
                              <select style={{...styles.input, fontSize:'13px', padding:'8px'}} onChange={e => setFormData({...formData, courseId: e.target.value})} value={formData.courseId}>
                      ...
                      <div style={{animation:'fadeIn 0.2s ease'}}>
                          ...
                          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'15px'}}>
                              ...
                              <div style={{border:'1px solid #eee', borderRadius:'8px', padding:'12px', boxShadow:'0 2px 5px rgba(0,0,0,0.01)'}}>
                                  ...
                                  <div>
                                      <input style={{...styles.input, padding:'8px', fontSize:'13px'}} value={formData.laundryToken} onChange={e=>setFormData({...formData, laundryToken:e.target.value})} placeholder="Laundry Token" />
                                  </div>
                              </div>
