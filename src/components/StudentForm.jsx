...
export default function StudentForm({ courseId, student = null, onSave, onCancel }) {
  ...
  const [formData, setFormData] = useState({
    ...
    mobile_locker_no: '',
    valuables_locker_no: '',
    laptop_locker_no: '', // ✅ Original Field
    laundry_token_no: '',
    discourse_language: 'Hindi', // ✅ Original Field
    special_seating: 'None'      // ✅ Original Field
  });
  ...
  const loadData = async () => {
      try {
          // 1. Course Info & Students (Restored Logic)
        ...
        <div style={{display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', gap:'15px', marginBottom:'15px'}}>
            <div>
                ...
                <div style={{display:'flex', alignItems:'center', border:'1px solid #ddd', borderRadius:'4px', padding:'0 5px'}}>
                    ...
                    <input list="s-list" name="full_name" value={formData.full_name} onChange={handleChange} style={{...styles.input, border:'none'}} required placeholder="Type or Select..." autoComplete="off"/>
                    <datalist id="s-list">{courseStudents.map(p=><option key={p.participant_id} value={p.full_name}/>)}</datalist>
                </div>
            </div>
