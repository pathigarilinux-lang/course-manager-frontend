...
export default function StudentForm({ courses, preSelectedRoom, clearRoom, userRole }) {
  ...
  // This prevents the "filter of undefined" crash
  useEffect(() => { 
      if (formData.courseId) {
          const syncData = () => {
              // 1. Fetch Participants (Guard against non-array response)
              fetch(`${API_URL}/courses/${formData.courseId}/participants`)
                  .then(res => res.json())
                  .then(data => setParticipants(Array.isArray(data) ? data : []))
                  .catch(e => { console.error(e); setParticipants([]); });

              // 2. Fetch Global Occupied (Guard against null properties)
              ...
              // 3. Fetch Room Occupancy (Guard against non-array)
  ...
  // ✅ SAFE ITERATION (Prevent crash if state is somehow not array)
