...
export default function CourseAdmin({ courses, refreshCourses, userRole }) { 
  ...
  const processDataRows = (rows) => {
    ...
    const map = { 
        ...
        // ✅ CRITICAL: Detect Phone Column (Phone, Mobile, Contact)
    ...
    try {
        const payload = { 
            students: students.map(s => ({ 
                ...
                // ✅ SEND AS 'phone' IF BACKEND EXPECTS IT
                phone: s.mobile 
            }))
        };
  ...
  return (
    <div style={styles.card}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'30px', borderBottom:'1px solid #eee', paddingBottom:'15px'}}>
        <div><h2 style={{margin:0, display:'flex', alignItems:'center', gap:'12px', color:'#2c3e50'}}><Database size={24} className="text-blue-600"/> Mission Control</h2><div style={{fontSize:'13px', color:'#666', marginTop:'5px'}}>System Administration</div></div>
        <div style={{display:'flex', gap:'8px', background:'#f1f3f5', padding:'5px', borderRadius:'10px'}}>
           {TABS.map(t => (<button key={t.id} onClick={()=>setActiveTab(t.id)} style={{display:'flex', alignItems:'center', gap:'8px', padding:'8px 16px', border:'none', borderRadius:'8px', background: activeTab===t.id ? 'white' : 'transparent', color: activeTab===t.id ? '#007bff' : '#666', fontWeight: activeTab===t.id ? 'bold' : 'normal', boxShadow: activeTab===t.id ? '0 2px 5px rgba(0,0,0,0.05)' : 'none', cursor:'pointer', transition:'all 0.2s'}}>{t.icon} {t.label}</button>))}
        </div>
      </div>
      ...
      {activeTab === 'courses' && (
        <div style={{display:'grid', gridTemplateColumns:'1fr 2fr', gap:'30px', animation:'fadeIn 0.3s ease'}}>
            <div style={{background:'linear-gradient(to bottom right, #ffffff, #f8f9fa)', padding:'25px', borderRadius:'12px', border:'1px solid #eee', height:'fit-content', boxShadow:'0 4px 12px rgba(0,0,0,0.03)'}}>
                <h3 style={{marginTop:0, display:'flex', alignItems:'center', gap:'10px', color: editingId ? '#e65100' : '#007bff'}}>{editingId ? <Edit size={20}/> : <PlusCircle size={20}/>}{editingId ? 'Edit Course' : 'Create Course'}</h3>
                <form onSubmit={editingId ? handleUpdateCourse : handleCreateCourse} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
                    ...
                    <button type="submit" style={{...styles.btn(true), background: editingId ? '#e65100' : '#28a745', color:'white', justifyContent:'center', padding:'12px', marginTop:'10px'}}>{editingId ? 'Update Course' : 'Create New Course'}</button>
                ...
                <div style={{maxHeight:'500px', overflowY:'auto', border:'1px solid #eee', borderRadius:'12px', boxShadow:'0 2px 8px rgba(0,0,0,0.03)'}}>
                    <table style={{width:'100%', borderCollapse:'collapse', fontSize:'14px'}}>
                        <thead style={{position:'sticky', top:0, background:'#f8f9fa'}}>
                            <tr style={{borderBottom:'2px solid #e9ecef'}}><th style={{padding:'15px', textAlign:'left', color:'#666'}}>Course Name</th><th style={{padding:'15px', textAlign:'left', color:'#666'}}>Teacher</th><th style={{padding:'15px', textAlign:'center', color:'#666'}}>Status</th><th style={{padding:'15px', textAlign:'right', color:'#666'}}>Action</th></tr>
                        </thead>
                        <tbody>
                            {courses.slice().reverse().map(c => {
                                ...
                                return (
                                    <tr key={c.course_id} style={{borderBottom:'1px solid #f0f0f0', background: isEditing ? '#fff3e0' : 'transparent'}}>
                                        <td style={{padding:'15px'}}><div style={{fontWeight:'bold', color:'#333'}}>{c.course_name.split('/')[0]}</div><div style={{fontSize:'12px', color:'#888', marginTop:'4px'}}>{new Date(c.start_date).toLocaleDateString()} - {new Date(c.end_date).toLocaleDateString()}</div></td>
                                        <td style={{padding:'15px', color:'#555'}}>{c.teacher_name}</td>
                                        <td style={{padding:'15px', textAlign:'center'}}><span style={{background: status.bg, color: status.color, padding:'4px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:'bold', display:'inline-block', minWidth:'80px', textAlign:'center'}}>{status.label}</span></td>
                                        <td style={{padding:'15px', textAlign:'right'}}><div style={{display:'flex', gap:'8px', justifyContent:'flex-end'}}><button onClick={()=>handleEditClick(c)} style={{padding:'8px', background:'white', color:'#e65100', border:'1px solid #ffe0b2', borderRadius:'6px', cursor:'pointer'}} title="Edit Course"><Edit size={16}/></button>{userRole === 'admin' && (<button onClick={()=>handleDeleteCourse(c.course_id, c.course_name)} style={{padding:'8px', background:'white', color:'#d32f2f', border:'1px solid #ffcdd2', borderRadius:'6px', cursor:'pointer'}} title="Delete Course"><Trash2 size={16}/></button>)}</div></td>
                                    </tr>
                                );
                            })}
                        </tbody>
      ...
      {activeTab === 'import' && (
        <div style={{maxWidth:'900px', margin:'0 auto', animation:'fadeIn 0.3s ease'}}>
          <div style={{marginBottom:'30px', background:'#e3f2fd', padding:'20px', borderRadius:'12px', border:'1px solid #bbdefb', display:'flex', alignItems:'center', gap:'20px'}}>
            ...
            <div style={{flex:1}}><label style={{display:'block', fontSize:'12px', fontWeight:'bold', color:'#0d47a1', marginBottom:'5px', textTransform:'uppercase'}}>Select Target Course</label><select style={{...styles.input, border:'none', fontSize:'16px', background:'transparent', fontWeight:'bold', padding:0, color:'#0056b3', outline:'none'}} value={selectedCourseForUpload} onChange={(e) => setSelectedCourseForUpload(e.target.value)}><option value="">-- Click to Select Course --</option>{courses.map(c => <option key={c.course_id} value={c.course_name}>{c.course_name}</option>)}</select></div>
          </div>
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'30px'}}>
              <div style={{border:'2px dashed #ccc', borderRadius:'12px', padding:'40px', textAlign:'center', background:'#f9f9f9', position:'relative', transition:'all 0.2s', ':hover':{borderColor:'#28a745', background:'#f0fff4'}}}>
                ...
                <div style={{pointerEvents:'none'}}><div style={{width:'60px', height:'60px', background:'#e9ecef', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 15px auto'}}><FileText size={30} color="#28a745"/></div><p style={{margin:'10px 0', color:'#333', fontWeight:'bold', fontSize:'16px'}}>Drop Excel or CSV</p><div style={{fontSize:'12px', color:'#888'}}>Supports .xlsx, .xls, .csv</div></div>
                <div style={{marginTop:'30px', pointerEvents:'auto'}}><button onClick={downloadTemplate} style={{background:'white', border:'1px solid #ddd', padding:'8px 15px', borderRadius:'20px', color:'#28a745', cursor:'pointer', fontSize:'12px', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px', margin:'0 auto'}}><Download size={12}/> Download Excel Template</button></div>
              </div>
              <div style={{border:'1px solid #eee', borderRadius:'12px', padding:'25px', background:'white', boxShadow:'0 2px 10px rgba(0,0,0,0.02)'}}>
                  ...
                  <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                      ...
                      <div style={{display:'flex', gap:'10px'}}><input style={styles.input} placeholder="Conf No" value={manualStudent.conf_no} onChange={e=>setManualStudent({...manualStudent, conf_no:e.target.value})} /><input style={styles.input} type="number" placeholder="Age" value={manualStudent.age} onChange={e=>setManualStudent({...manualStudent, age:e.target.value})} /></div>
                      <div style={{display:'flex', gap:'10px'}}><select style={styles.input} value={manualStudent.gender} onChange={e=>setManualStudent({...manualStudent, gender:e.target.value})}><option>Male</option><option>Female</option></select><input style={styles.input} placeholder="Courses (S:1)" value={manualStudent.courses_info} onChange={e=>setManualStudent({...manualStudent, courses_info:e.target.value})} /></div>
          ...
          {mappingReport && (
              <div style={{marginTop:'20px', padding:'15px', background:'#e8f5e9', border:'1px solid #c8e6c9', borderRadius:'8px'}}>
                  <div style={{display:'flex', alignItems:'center', gap:'10px', marginBottom:'10px', color:'#2e7d32', fontWeight:'bold'}}><CheckCircle size={20}/> Smart Mapping Confirmation</div>
          ...
          {students.length > 0 && (
            <div style={{marginTop:'40px', borderTop:'1px solid #eee', paddingTop:'20px'}}>
               <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'15px'}}>
                  ...
                  <div style={{display:'flex', gap:'10px'}}><button onClick={()=>{setStudents([]); setMappingReport(null);}} style={styles.btn(false)}>Clear All</button><button onClick={saveToDatabase} style={{...styles.btn(true), background:'#28a745', color:'white', padding:'10px 25px'}}><Save size={16}/> Save to Database</button></div>
               </div>
      ...
      {activeTab === 'backup' && userRole === 'admin' && ( 
          <div style={{textAlign:'center', padding:'60px 40px', animation:'fadeIn 0.3s ease'}}>
              <div style={{width:'80px', height:'80px', background:'#f8f9fa', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 20px auto'}}><Database size={40} color="#6c757d"/></div>
              <h3 style={{fontSize:'24px', margin:'0 0 10px 0'}}>System Backup</h3>
              <p style={{color:'#666', marginBottom:'40px', maxWidth:'500px', margin:'0 auto 40px auto', lineHeight:'1.6'}}>Create a complete snapshot of your entire database including all courses, student records, room assignments, and financial data.</p>
              <button onClick={handleDownloadBackup} style={{...styles.btn(true), background:'#343a40', color:'white', padding:'15px 40px', fontSize:'16px', borderRadius:'30px', boxShadow:'0 5px 15px rgba(0,0,0,0.1)'}}><Download size={18}/> Download Full Backup (.json)</button>
          </div>
      )}
          ...
          <div style={{maxWidth:'900px', margin:'0 auto', animation:'fadeIn 0.3s ease'}}>
              <div style={{display:'flex', gap:'15px', marginBottom:'30px'}}>
                  <div style={{flex:1, position:'relative'}}>
                      ...
                      <input style={{...styles.input, padding:'15px 15px 15px 50px', fontSize:'16px', borderRadius:'30px', border:'1px solid #ddd', width:'100%', boxSizing:'border-box'}} placeholder="Search Name or Conf No across ALL courses..." value={globalSearch} onChange={e=>setGlobalSearch(e.target.value)} onKeyDown={e=>e.key==='Enter' && handleGlobalSearch()} />
                  </div>
                  <button onClick={handleGlobalSearch} style={{...styles.btn(true), background:'#007bff', color:'white', padding:'0 30px', borderRadius:'30px'}}>{isSearching ? 'Searching...' : 'Search'}</button>
              </div>
              {globalResults.length > 0 ? (
                  <div style={{background:'white', borderRadius:'12px', border:'1px solid #eee', overflow:'hidden', boxShadow:'0 4px 10px rgba(0,0,0,0.03)'}}>
                      <table style={{width:'100%', borderCollapse:'collapse'}}>
                          <thead style={{background:'#f8f9fa'}}><tr><th style={{padding:'15px', textAlign:'left', borderBottom:'1px solid #ddd'}}>Name</th><th style={{padding:'15px', textAlign:'left', borderBottom:'1px solid #ddd'}}>Conf No</th><th style={{padding:'15px', textAlign:'left', borderBottom:'1px solid #ddd'}}>Course</th><th style={{padding:'15px', textAlign:'left', borderBottom:'1px solid #ddd'}}>Status</th></tr></thead>
                          <tbody>{globalResults.map((r, i) => (<tr key={i} style={{borderBottom:'1px solid #f0f0f0'}}><td style={{padding:'15px', fontWeight:'bold'}}>{r.full_name}</td><td style={{padding:'15px', color:'#555'}}>{r.conf_no}</td><td style={{padding:'15px'}}><span style={{background:'#e3f2fd', color:'#0d47a1', padding:'3px 10px', borderRadius:'12px', fontSize:'12px', fontWeight:'bold'}}>{r.courseName}</span></td><td style={{padding:'15px'}}>{r.status}</td></tr>))}</tbody>
                      </table>
                  </div>
