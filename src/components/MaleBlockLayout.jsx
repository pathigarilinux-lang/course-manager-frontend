...
export default function MaleBlockLayout({ rooms, occupancy, onRoomClick }) {
    ...
    const BlockSection = ({ title, color, children }) => (
        <div style={{border:`2px solid ${color}`, borderRadius:'10px', padding:'15px', background:'#fff'}}>
            <h3 style={{marginTop:0, color:color, borderBottom:`1px solid ${color}33`, paddingBottom:'5px', fontSize:'16px'}}>{title}</h3>
            {children}
        </div>
    );
    ...
    return (
        <div style={{display:'flex', flexDirection:'column', gap:'30px'}}>
            ...
            {/* --- OTHERS --- */}
            {getOthers().length > 0 && (
                <BlockSection title="Other Rooms" color="#555">
