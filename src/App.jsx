...
// --- 2. CHECK-IN FORM (Auto-Fill Conf No) ---
function CheckInForm({ courses }) {
  ...
  useEffect(() => {
    if (formData.courseId) {
      fetch(`${API_URL}/courses/${formData.courseId}/participants`)
        .then(res => res.json()).then(data => setParticipants(Array.isArray(data) ? data : []));
    }
  }, [formData.courseId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('✅ Success!'); setFormData(prev => ({ ...prev, participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', pagodaCell: '', laptop: '', confNo: '', dhammaSeat: '', specialSeating: '', language: 'English' }));
...
// --- 3. MANAGE STUDENTS (Reset, Delete, Dining, Seating) ---
function ParticipantList({ courses, refreshCourses }) {
  ...
  const filtered = participants.filter(p => p.full_name.toLowerCase().includes(search.toLowerCase())); const selectedCourseName = courses.find(c => c.course_id == courseId)?.course_name || 'Course';
