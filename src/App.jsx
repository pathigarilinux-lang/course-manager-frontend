...
// --- 1. DASHBOARD (UPDATED: Language Summary) ---
      ...
      {stats && selectedCourse && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', animation: 'fadeIn 0.5s' }}>
          ...
          <div style={{display:'grid', gap:'20px'}}>
             ...
             {/* NEW: Discourse Language Summary */}
             <div style={cardStyle}>
                <h3 style={{marginTop:0}}>Discourse Language Summary</h3>
                {stats.languages && stats.languages.length > 0 ? (
                  <table style={{width:'100%', fontSize:'14px'}}>
                    <thead><tr style={{textAlign:'left', borderBottom:'1px solid #eee'}}><th>Language</th><th style={{textAlign:'right'}}>Count</th></tr></thead>
                    <tbody>
                      {stats.languages.map((l, i) => (
                        <tr key={i} style={{borderBottom:'1px solid #f4f4f4'}}>
                          <td style={{padding:'8px 0'}}>{l.discourse_language || 'Unknown'}</td>
                          <td style={{padding:'8px 0', textAlign:'right', fontWeight:'bold'}}>{l.count}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ) : <p style={{color:'#888'}}>No checked-in students yet.</p>}
             </div>
          </div>
        </div>
      )}
...
// --- 3. MANAGE STUDENTS (List + Export + Sort + Dhamma) ---
