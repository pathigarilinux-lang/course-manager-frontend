        ...
        <div style={{maxHeight:'500px', overflowY:'auto'}}>
           <table style={{width:'100%', borderCollapse:'collapse', fontSize:'14px'}}>
             <thead><tr style={{textAlign:'left', borderBottom:'2px solid #eee', background:'#f9f9f9'}}>
               ...
               <th style={{padding:'10px'}}>Special</th><th style={{padding:'10px'}}>Evening Food</th><th style={{padding:'10px'}}>Medical</th><th style={{padding:'10px'}}>Action</th>
             </tr></thead>
             <tbody>
               {filtered.map(p => (
    ...
    <div>
      ...
      {stats && selectedCourse ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', animation: 'fadeIn 0.5s' }}>
          ...
          {/* MIDDLE: Text Stats */}
          <div style={cardStyle}>
              <h3 style={{marginTop:0}}>Occupancy & Discourse</h3>
...
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  const [participants, setParticipants] = useState([]); const [rooms, setRooms] = useState([]); const [occupancy, setOccupancy] = useState([]); const [selectedStudent, setSelectedStudent] = useState(null); const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', language: 'English', pagodaCell: '', laptop: 'No', confNo: '', dhammaSeat: '', specialSeating: 'None', seatType: 'Chair' }); const [status, setStatus] = useState('');
...
function ParticipantList({ courses, refreshCourses }) {
  ...
  // --- NEW AUTO-ASSIGN LOGIC (Male 10x8 Left-to-Right) ---
  ...
  const handleAutoAssign = async () => {
    ...
    // Male: A1...J1, then A2...J2 (Standard Left-to-Right)
    males.forEach((p, i) => { 
