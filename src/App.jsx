// App.jsx (fixed — controlled selects, safe fetch guards, clear participants on empty id)
...
function ATPanel({ courses }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/courses/${courseId}/participants`)
      .then(res => res.json()).then(setParticipants).catch(console.error);
  }, [courseId]);
...
function GlobalAccommodationManager({ courses, onRoomClick }) {
  ...
  const loadData = () => { setLoading(true); fetch(`${API_URL}/rooms`).then(res => res.json()).then(data => setRooms(Array.isArray(data) ? data : [])).catch(console.error); fetch(`${API_URL}/rooms/occupancy`).then(res => res.json()).then(data => { setOccupancy(Array.isArray(data) ? data : []); setLoading(false); }).catch(() => setLoading(false)); };
  useEffect(loadData, []);
...
function ProcessFlowDashboard({ courses }) {
    ...
    const refresh = () => { if (!courseId) { setData([]); return; } fetch(`${API_URL}/courses/${courseId}/participants`).then(r=>r.json()).then(setData).catch(console.error); };
...
function ArrivalDesk({ courses }) {
    ...
    useEffect(() => {
      if (!courseId) { setParticipants([]); return; }
      fetch(`${API_URL}/courses/${courseId}/participants`).then(r=>r.json()).then(setParticipants).catch(console.error);
    }, [courseId]);
      ...
      <div style={cardStyle}>
        ...
        <div style={{maxHeight:'400px', overflowY:'auto'}}>
          <table style={{width:'100%'}}>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td style={{padding:'10px'}} colSpan="2">{courseId ? 'No participants found.' : 'Please select a course.'}</td></tr>
              ) : (
                filtered.map(p => (
                  <tr key={p.participant_id} style={{borderBottom:'1px solid #eee'}}>
                    <td style={{padding:'10px'}}><strong>{p.full_name}</strong></td>
...
function ProcessDesk({ courses }) {
    ...
    const refresh = () => { if (!courseId) { setParticipants([]); return; } fetch(`${API_URL}/courses/${courseId}/participants`).then(r=>r.json()).then(setParticipants).catch(console.error); };
    useEffect(refresh, [courseId]);
...
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
    ...
    useEffect(() => { fetch(`${API_URL}/rooms`).then(r=>r.json()).then(setRooms).catch(console.error); fetch(`${API_URL}/rooms/occupancy`).then(r=>r.json()).then(setOccupancy).catch(console.error); }, []);
    ...
    useEffect(() => { if(!formData.courseId) { setParticipants([]); return; } fetch(`${API_URL}/courses/${formData.courseId}/participants`).then(r=>r.json()).then(setParticipants).catch(console.error); }, [formData.courseId]);
    ...
    return ( <div style={cardStyle}> <h2>📝 Student Onboarding Form</h2> <form onSubmit={handleSubmit} style={{ maxWidth: '900px' }}> <div style={{background:'#f9f9f9', padding:'20px', borderRadius:'10px', marginBottom:'20px'}}> <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}> <div><label style={labelStyle}>1. Select Course</label><select style={inputStyle} value={formData.courseId} onChange={e => setFormData({...formData, courseId: e.target.value})}><option value="">-- Select --</option>{courses.map(c => <option key={c.course_id} value={c.course_id}>{c.course_name}</option>)}</select></div> <div><label style={labelStyle}>2. Select Student (Interviewed Only)</label><select style={inputStyle} value={formData.participantId} onChange={handleStudentChange} disabled={!formData.courseId} required><option value="">-- Select --</option>{eligibleStudents.map(p => <option key={p.participant_id} value={p.participant_id}>#{p.token_number} - {p.full_name}</option>)}</select></div> </div> 
...
function ExpenseTracker({ courses }) {
  ...
  useEffect(() => { if (!courseId) { setParticipants([]); return; } fetch(`${API_URL}/courses/${courseId}/participants`).then(res => res.json()).then(data => setParticipants(Array.isArray(data)?data:[])).catch(console.error); }, [courseId]);
  ...
  return (
    <div style={cardStyle}>
      ...
      <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
        <select style={inputStyle} value={courseId} onChange={e => setCourseId(e.target.value)} required> <option value="">-- 1. Select Course --</option> {courses.map(c => <option key={c.course_id} value={c.course_id}>{c.course_name}</option>)} </select>
        <select style={inputStyle} value={selectedStudentId} onChange={e => setSelectedStudentId(e.target.value)} disabled={!courseId} required> <option value="">-- 2. Select Student --</option> {participants.map(p => <option key={p.participant_id} value={p.participant_id}>{p.full_name}</option>)} </select>
...
function ManualStudentForm({ courses, setView }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:'15px', maxWidth:'600px'}}>
       ...
       <select style={inputStyle} value={formData.courseId} onChange={e => setFormData({...formData, courseId: e.target.value})} required>
       ...
       <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'10px'}}><div><label>Gender</label><select style={inputStyle} value={formData.gender} onChange={e=>setFormData({...formData, gender: e.target.value})}><option value="">Select</option><option>Male</option><option>Female</option></select></div><div><label>Age</label><input style={inputStyle} type="number" value={formData.age} onChange={e=>setFormData({...formData, age: e.target.value})} /></div><div><label>Courses Info</label><input style={inputStyle} value={formData.coursesInfo} onChange={e=>setFormData({...formData, coursesInfo: e.target.value})} placeholder="S:0 L:0" /></div></div>
