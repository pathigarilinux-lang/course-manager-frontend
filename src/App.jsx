  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'15px', marginBottom:'20px'}}>
        ...
        {/* NEW: Room Allocation by Course */}
        <div style={{padding:'12px', background:'#e8f5e9', borderRadius:'8px', borderLeft:'5px solid #2e7d32', gridColumn: 'span 2'}}>
          <div style={{fontSize:'14px', fontWeight:'bold', color:'#2e7d32', marginBottom:'5px'}}>Room Allocation by Course</div>
          <div style={{fontSize:'11px', color:'#333', marginTop:'5px', display:'flex', flexWrap:'wrap', gap:'10px'}}>
             {Object.entries(courseBreakdown).length > 0 ? Object.entries(courseBreakdown).map(([name, count]) => (
                <span key={name} style={{background:'white', padding:'4px 8px', borderRadius:'4px', border:'1px solid #ccc', fontWeight:'bold'}}>
                   {name}: {count} rooms
                </span>
             )) : "No rooms allocated."}
          </div>
...
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  ...
  if (selectedStudent && selectedStudent.gender) {
      ...
      if (gender === 'male') availableRooms = availableRooms.filter(r => r.room_no.startsWith('3')); // 3-series = Male
      else if (gender === 'female') availableRooms = availableRooms.filter(r => r.room_no.startsWith('2')); // 2-series = Female
  }
