...
// --- 1. DASHBOARD (UPDATED: Applicant Types) ---
function Dashboard({ courses }) {
  ...
  const arrivalData = [
    { name: 'Arrived', count: stats.arrived }, { name: 'Pending', count: stats.no_response }, { name: 'Cancelled', count: stats.cancelled }
  ];
  
  // New Data for Student Type
  const typeData = [
    { name: 'Old Students', value: stats.old_students, color: '#8884d8' },
    { name: 'New Students', value: stats.new_students, color: '#82ca9d' },
    { name: 'Servers (DS)', value: stats.servers, color: '#ffc658' }
  ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{height:'250px', width:'100%'}}>
            <ResponsiveContainer width="100%" height="100%"><BarChart data={arrivalData}><XAxis dataKey="name" /><YAxis /><Tooltip /><Legend /><Bar dataKey="count" fill="#0088FE" barSize={50} /></BarChart></ResponsiveContainer>
          </div>
        </div>
        {/* Chart 2: Applicant Type */}
        <div style={cardStyle}>
          <h3 style={{marginTop:0}}>Applicant Types (Checked-In)</h3>
          <div style={{height:'250px', width:'100%'}}>
             <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={typeData} cx="50%" cy="50%" outerRadius={80} dataKey="value" label>{typeData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip /><Legend /></PieChart></ResponsiveContainer>
          </div>
        </div>
        {/* Counts */}
        <div style={cardStyle}>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'10px', marginTop:'10px'}}>
             <div style={{padding:'10px', background:'#e3f2fd', borderRadius:'5px'}}>Confirmed: <strong>{stats.arrived}</strong></div>
             <div style={{padding:'10px', background:'#fff3e0', borderRadius:'5px'}}>Pending: <strong>{stats.no_response}</strong></div>
             <hr />
             <div>Old Students (OM/OF): <strong>{stats.old_students}</strong></div>
             <div>New Students (NM/NF): <strong>{stats.new_students}</strong></div>
             <div>Servers (SM/SF): <strong>{stats.servers}</strong></div>
          </div>
        </div>
      </div>
    </div>
...
// --- 2. CHECK-IN FORM (UPDATED: Special Seating) ---
function CheckInForm({ courses }) {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setStatus('Submitting...'); try { const res = await fetch(`${API_URL}/check-in`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) }); const data = await res.json(); if (!res.ok) throw new Error(data.error || "Error"); setStatus('✅ Success!'); setFormData(prev => ({ ...prev, participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', pagodaCell: '', laptop: '', confNo: '', dhammaSeat: '', specialSeating: '', language: 'English' })); fetch(`${API_URL}/courses/${formData.courseId}/participants`).then(res => res.json()).then(data => setParticipants(data)); } catch (err) { setStatus(`❌ ${err.message}`); } };
    ...
    <div style={cardStyle}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '900px' }}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:'10px'}}>
          <div><label style={labelStyle}>Conf No</label><input style={inputStyle} value={formData.confNo} onChange={e => setFormData({...formData, confNo: e.target.value})} placeholder="e.g. OM23" /></div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'10px'}}>
           ...
           <div><label style={labelStyle}>Special Seating</label>
...
// --- 3. MANAGE STUDENTS (Includes Dining Sheet) ---
function ParticipantList({ courses }) {
  ...
  const loadStudents = () => { if (!courseId) return; fetch(`${API_URL}/courses/${courseId}/participants`).then(res => res.json()).then(data => setParticipants(Array.isArray(data) ? data : [])); };
  useEffect(loadStudents, [courseId]);
  const handleDelete = async (id) => { if (!window.confirm("Delete?")) return; await fetch(`${API_URL}/participants/${id}`, { method: 'DELETE' }); loadStudents(); };
