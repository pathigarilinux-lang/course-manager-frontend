...
// --- NEW COMPONENT: REPORTS & PRINTING ---
function ReportsPanel({ courses }) {
  const [mode, setMode] = useState('dining'); // 'dining' or 'invoice'
  ...
  useEffect(() => {
    ...
    // Fetch Students for Dining Sheet
  ...
  // Fetch Expenses for Invoice
  useEffect(() => {
    if (studentId) {
      fetch(`${API_URL}/participants/${studentId}/expenses`)
        .then(res => res.json())
  ...
  const selectedStudent = data.find(p => p.participant_id == studentId);
    ...
    <div style={cardStyle}>
      ...
      <div className="no-print" style={{marginBottom: '20px', borderBottom: '1px solid #eee', paddingBottom: '15px'}}>
        ...
        <div style={{display: 'flex', gap: '10px', marginBottom: '15px'}}>
          <button onClick={() => setMode('dining')} style={quickBtnStyle(mode==='dining')}>🍽️ Dining Sheet</button>
          <button onClick={() => setMode('invoice')} style={quickBtnStyle(mode==='invoice')}>🧾 Student Invoice</button>
        </div>
        ...
        <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
          ...
          {mode === 'invoice' && (
            <select style={inputStyle} onChange={e => setStudentId(e.target.value)} disabled={!courseId}>
              <option value="">-- Select Student --</option>
              {data.map(p => <option key={p.participant_id} value={p.participant_id}>{p.full_name}</option>)}
            </select>
          )}

          <button onClick={printPage} disabled={!courseId} style={{...btnStyle(true), background: '#28a745', color: 'white'}}>🖨️ Print Now</button>
        </div>
      </div>
      ...
      {/* 2. INVOICE VIEW */}
      {mode === 'invoice' && studentId && selectedStudent && (
        <div className="print-area" style={{maxWidth: '800px', margin: '0 auto', border: '1px solid #eee', padding: '40px'}}>
          <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '40px'}}>
            ...
            <div style={{textAlign: 'right'}}>
              ...
              <p>Room: {selectedStudent.room_no}</p>
          ...
          <table style={{width: '100%', borderCollapse: 'collapse', marginBottom: '30px'}}>
            <thead>
              <tr style={{background: '#f9f9f9', borderBottom: '2px solid #333'}}>
          ...
          <div style={{textAlign: 'right', marginTop: '20px'}}>
            <h3>Total Due: ₹{expenses.reduce((sum, item) => sum + parseFloat(item.amount), 0)}</h3>
          </div>
          
          <div style={{marginTop: '60px', borderTop: '1px solid #000', width: '200px', textAlign: 'center', paddingTop: '5px'}}>
            Signature
          </div>
        </div>
      )}

      {!courseId && <p style={{textAlign: 'center', color: '#888', padding: '50px'}}>Select a course to view report.</p>}
    </div>
  );
}

// --- OTHER COMPONENTS (UNCHANGED - PASTE THEM BELOW) ---
// (Dashboard, ExpenseTracker, CheckInForm, CreateCourseForm, UploadParticipants, ParticipantList)
// ... [Include all other components from the previous step here] ...
...
function CheckInForm({ courses }) {
  const [participants, setParticipants] = useState([]); const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '' }); const [status, setStatus] = useState('');
