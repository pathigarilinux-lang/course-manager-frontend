...
// --- 0. TEACHER ASSISTANT PANEL (New Feature) ---
...
function GlobalAccommodationManager({ courses, onRoomClick }) {
  ...
  const occupiedMap = {}; const maleBreakdown = {}; const femaleBreakdown = {}; const unmappedParticipants = [];
  const safeRooms = rooms || []; const validRoomSet = new Set(safeRooms.map(r => normalize(r.room_no)));

  (occupancy || []).forEach(p => { if(p.room_no) { const normRoom = normalize(p.room_no); const rObj = safeRooms.find(r => normalize(r.room_no)===normRoom); if (rObj) { occupiedMap[normRoom] = p; const c = p.course_name || 'Unknown'; if(rObj.gender_type==='Male') maleBreakdown[c]=(maleBreakdown[c]||0)+1; else femaleBreakdown[c]=(femaleBreakdown[c]||0)+1; } else unmappedParticipants.push(p); } });
  ...
  <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'15px', marginBottom:'20px'}}> <div style={{padding:'12px', background:'#e3f2fd', borderRadius:'8px', borderLeft:'5px solid #1565c0'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#1565c0', marginBottom:'5px'}}>MALE WING</div> <div>Occ: <strong>{maleOcc}</strong> / Free: <strong>{maleFree}</strong></div> <div style={{marginTop:'5px', fontSize:'10px', color:'#555'}}>{Object.entries(maleBreakdown).map(([n, c]) => <div key={n}>{n.substring(0,15)}: {c}</div>)}</div> </div> <div style={{padding:'12px', background:'#fce4ec', borderRadius:'8px', borderLeft:'5px solid #ad1457'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#ad1457', marginBottom:'5px'}}>FEMALE WING</div> <div>Occ: <strong>{femaleOcc}</strong> / Free: <strong>{femaleFree}</strong></div> <div style={{marginTop:'5px', fontSize:'10px', color:'#555'}}>{Object.entries(femaleBreakdown).map(([n, c]) => <div key={n}>{n.substring(0,15)}: {c}</div>)}</div> </div> <div style={{padding:'12px', background:'#e8f5e9', borderRadius:'8px', borderLeft:'5px solid #2e7d32'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#2e7d32', marginBottom:'5px'}}>TOTAL SUMMARY</div> <div>Total Occupied: <strong>{maleOcc + femaleOcc}</strong></div> <div>Total Free: <strong>{maleFree + femaleFree}</strong></div> </div> </div>
...
// --- 3. STUDENT FORM (Restricted Inputs + TA Alerts) ---
