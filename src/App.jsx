...
function ParticipantList({ courses, refreshCourses }) {
    ...
    if (viewMode === 'seating') {
        const maleMap={}; const femaleMap={}; participants.forEach(p => { if(p.dhamma_hall_seat_no) { if(p.gender==='Male') maleMap[p.dhamma_hall_seat_no]=p; else femaleMap[p.dhamma_hall_seat_no]=p; } });
    ...
    if (viewMode === 'dining') {
            ...
            tableRows.push(<tr key="no-data"><td colSpan="5" style={{textAlign: 'center', padding: '20px'}}>No students assigned to dining seats yet.</td></tr>);
             ...
             tableRows.push(
                <tr key="header" style={{background:'#f9f9f9', borderBottom:'2px solid #333'}}>
                    ...
                    <th style={{padding:'10px'}}>Name</th>
                    <th style={{padding:'10px'}}>Conf No</th>
                    <th style={{padding:'10px'}}>Type</th>
                    <th style={{padding:'10px'}}>Food/Medical</th>
                </tr>
            );

            sortedSeats.forEach(seatNo => {
                const item = diningMap[seatNo];
                const p = item.p;
                const bgColor = item.gender === 'Male' ? '#e3f2fd' : '#fce4ec';

                tableRows.push(
                    <tr key={seatNo} style={{borderBottom:'1px solid #eee', background: bgColor}}>
                        <td style={{padding:'8px', fontWeight:'bold', color:'#333'}}>{seatNo}</td>
                        <td style={{padding:'8px'}}>{p.full_name}</td>
                        <td style={{padding:'8px', fontSize:'12px', color:'#555'}}>{p.conf_no}</td>
                        <td style={{padding:'8px', fontSize:'12px'}}>{p.dining_seat_type || '-'}</td>
                        <td style={{padding:'8px', fontSize:'12px', color: (p.evening_food || p.medical_info) ? 'red' : '#000'}}>
                            {p.evening_food && `Food: ${p.evening_food}`}
                            {p.medical_info && ` Med: ${p.medical_info}`}
                            {(!p.evening_food && !p.medical_info) && 'None'}
                        </td>
                    </tr>
                );
            });
        ...
        return (
            <div style={cardStyle}>
                <div className="no-print" style={{marginBottom:'10px', display:'flex', gap:'10px'}}>
                    <button onClick={() => setViewMode('list')} style={btnStyle(false)}>← Back to List</button>
                    <button onClick={() => window.print()} style={{...quickBtnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Dining Sheet</button>
                </div>
                <div className="print-area">
                    <h2 style={{textAlign:'center'}}>Dining Hall Seating Sheet - {selectedCourseName}</h2>
...
function UploadParticipants({ courses, setView }) { const [courseId, setCourseId] = useState(''); const [csvFile, setCsvFile] = useState(null); const [preview, setPreview] = useState([]); const [status, setStatus] = useState(''); 
  ...
  return ( <div><h3>Upload CSV</h3><div style={{maxWidth:'500px'}}><div style={{marginBottom:'10px'}}><label>Select Course:</label><select style={inputStyle} onChange={e => setCourseId(e.target.value)}><option value="">-- Select --</option>{courses.map(c => <option key={c.course_id} value={c.course_id}>{c.course_name}</option>)}</select></div><div style={{marginBottom:'10px'}}><input type="file" accept=".csv" onChange={handleFileChange} /></div>{status && <p style={{padding:'10px', background:'#e3f2fd', borderRadius:'4px', marginBottom:'10px', color: status.startsWith('❌') ? 'red' : 'green'}}>{status}</p>}<button onClick={handleUpload} disabled={!csvFile || !courseId || preview.length===0} style={{...quickBtnStyle(true), width:'100%', background: preview.length>0?'#28a745':'#ccc'}}>Upload</button></div></div> );
}
