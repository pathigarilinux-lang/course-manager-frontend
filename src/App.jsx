...
export default function App() {
  ...
  return (
    <div className="app-container" style={{ fontFamily: 'Segoe UI, sans-serif', padding: '20px', backgroundColor: '#f4f7f6', minHeight: '100vh' }}>
      ...
      {view === 'room-view' && <RoomDashboard courses={courses} />}
...
// --- 1. ROOM DASHBOARD (DYNAMIC + EDIT + STATS) ---
function RoomDashboard({ courses }) {
  const [courseId, setCourseId] = useState('');
  const [participants, setParticipants] = useState([]);
  const [rooms, setRooms] = useState([]);
  const [editingRoom, setEditingRoom] = useState(null); // For Swap
  const [newRoom, setNewRoom] = useState({ roomNo: '', type: 'Male' }); // For Add
  ...
  const handleAddRoom = async (e) => {
    ...
    setNewRoom({ roomNo: '', type: 'Male' });
    loadData();
  };

  const handleDeleteRoom = async (id) => { if(window.confirm("Delete Room?")) { await fetch(`${API_URL}/rooms/${id}`, { method: 'DELETE' }); loadData(); } };

  const handleSwapSave = async () => {
    if (!editingRoom || !editingRoom.p) return;
    await fetch(`${API_URL}/participants/${editingRoom.p.participant_id}`, {
  ...
  // Logic to Map Participants to Rooms
  const occupiedRooms = {};
  participants.forEach(p => { if(p.room_no && p.status==='Arrived') occupiedRooms[p.room_no.trim().toUpperCase()] = p; });

  // Stats Logic
  let maleTotal=0, femaleTotal=0, maleOcc=0, femaleOcc=0, oldOcc=0, newOcc=0;
  rooms.forEach(r => {
    if (r.gender_type === 'Male') maleTotal++; else femaleTotal++;
    const p = occupiedRooms[r.room_no.toUpperCase()];
    if (p) {
      if (r.gender_type === 'Male') maleOcc++; else femaleOcc++;
      if (p.conf_no && (p.conf_no.startsWith('O') || p.conf_no.startsWith('S'))) oldOcc++; else newOcc++;
    }
  });
  ...
  return (
    <div style={cardStyle}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:'20px', alignItems:'center', flexWrap:'wrap', gap:'10px'}}>
        <h2 style={{margin:0}}>🛏️ Accommodation</h2>
        <select style={inputStyle} onChange={e => setCourseId(e.target.value)} value={courseId}>{courses.map(c => <option key={c.course_id} value={c.course_id}>{c.course_name}</option>)}</select>
      </div>

      {/* Stats Bar */}
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'10px', marginBottom:'20px'}}>
        <div style={{padding:'10px', background:'#e3f2fd', borderRadius:'6px'}}><div>Male Free: <strong>{maleTotal - maleOcc}</strong></div><div style={{fontSize:'11px'}}>Total: {maleTotal}</div></div>
        <div style={{padding:'10px', background:'#fce4ec', borderRadius:'6px'}}><div>Female Free: <strong>{femaleTotal - femaleOcc}</strong></div><div style={{fontSize:'11px'}}>Total: {femaleTotal}</div></div>
        <div style={{padding:'10px', background:'#e8f5e9', borderRadius:'6px'}}><div>Occupied: <strong>{maleOcc + femaleOcc}</strong></div><div style={{fontSize:'11px'}}>Old: {oldOcc} | New: {newOcc}</div></div>
      </div>

      {/* Add Room Form */}
      <div style={{marginBottom:'20px', padding:'10px', background:'#f9f9f9', borderRadius:'6px', display:'flex', gap:'10px', alignItems:'center'}}>
        <span style={{fontSize:'12px', fontWeight:'bold'}}>ADD ROOM:</span>
        <input style={{...inputStyle, width:'100px'}} placeholder="Room No" value={newRoom.roomNo} onChange={e=>setNewRoom({...newRoom, roomNo:e.target.value})} />
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
        {['Male', 'Female'].map(gender => (
          <div key={gender} style={{border: gender==='Male'?'1px solid #90caf9':'1px solid #f48fb1', borderRadius:'8px', padding:'10px'}}>
            <h3 style={{textAlign:'center', background: gender==='Male'?'#e3f2fd':'#fce4ec', margin:'0 0 10px 0', padding:'5px'}}>{gender.toUpperCase()} WING</h3>
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'8px'}}>
              {rooms.filter(r => r.gender_type === gender).map(r => {
                const p = occupiedRooms[r.room_no.toUpperCase()];
                return (
                  <div key={r.room_id} onClick={() => p && setEditingRoom({ p, newRoomNo: p.room_no })}
                    style={{
                      border: p ? '1px solid #ef9a9a' : '1px solid #a5d6a7',
                      background: p ? '#ffebee' : '#e8f5e9',
                      borderRadius: '6px', padding: '8px', textAlign: 'center',
                      cursor: p ? 'pointer' : 'default', position:'relative'
                    }}
                  >
                    <div style={{fontWeight:'bold', fontSize:'14px'}}>{r.room_no}</div>
                    {p ? (
                      <div style={{fontSize:'11px', color:'#c62828', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                        {p.full_name.split(' ')[0]}<br/><span style={{fontSize:'9px'}}>({p.conf_no})</span>
                      </div>
                    ) : (
                       <button onClick={()=>handleDeleteRoom(r.room_id)} style={{fontSize:'9px', border:'none', background:'none', color:'red', cursor:'pointer'}}>x</button>
                    )}
                  </div>
                );
              })}
      ...
      {editingRoom && (
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center'}}>
          <div style={{background:'white', padding:'20px', borderRadius:'10px', width:'300px'}}>
            <h3>Swap Room</h3>
            <p>Student: <strong>{editingRoom.p.full_name}</strong></p>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '900px' }}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:'10px'}}>
          <div>
            ...
            <input list="room-list" style={inputStyle} value={formData.roomNo} onChange={e => setFormData({...formData, roomNo: e.target.value})} required />
            <datalist id="room-list">{rooms.map(r => <option key={r.room_id} value={r.room_no} />)}</datalist>
          </div>
