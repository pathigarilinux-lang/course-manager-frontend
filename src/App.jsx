...
function Dashboard({ courses }) {
  ...
  return (
    <div>
      ...
      {/* SCROLLING MARQUEE */}
      <div style={{background:'#333', color:'white', padding:'5px', marginBottom:'20px', overflow:'hidden', whiteSpace:'nowrap', borderRadius:'4px'}}>
          <marquee>📢 Welcome to Zero Day! Please ensure all medical alerts are checked. Check-in closes at 7:00 PM.</marquee>
      </div>
...
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  const [participants, setParticipants] = useState([]); const [rooms, setRooms] = useState([]); const [occupancy, setOccupancy] = useState([]); const [selectedStudent, setSelectedStudent] = useState(null); const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', language: 'English', pagodaCell: '', laptop: 'No', confNo: '', specialSeating: 'None', seatType: 'Floor' }); const [status, setStatus] = useState('');
...
function ParticipantList({ courses, refreshCourses }) {
  ...
  const handleAutoAssign = async () => {
    ...
    // Sort Logic: Old (Seniority), New (Age)
    ...
    // Logic: Fill Row 1 (A1, B1, C1...), then Row 2 (A2...)
    // Male: 10 Cols (A-J), 8 Rows (1-8)
    males.forEach((p, i) => { 
        if (i < MALE_COLS * MALE_ROWS) {
            const colIdx = i % MALE_COLS; // Column A, B, C...
            const rowIdx = Math.floor(i / MALE_COLS) + 1; // Row 1, 2, 3...
  ...
  if (viewMode === 'seating') { 
    ...
    // Render Letter = Column, Number = Row (A1, A2...)
    const renderGrid = (map, cols, rows) => { let grid = []; for (let r = 0; r < rows; r++) { let rowCells = []; for (let c = 0; c < cols; c++) { const colChar = String.fromCharCode(65 + c); const seatLabel = `${colChar}${r+1}`; const student = map[seatLabel]; rowCells.push(<SeatBox key={seatLabel} p={student} label={seatLabel} />); } grid.push(<div key={r} style={{display:'grid', gridTemplateColumns:`repeat(${cols}, 1fr)`, gap:'5px', marginBottom:'5px'}}>{rowCells}</div>); } return grid; };
