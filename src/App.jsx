    ...
    <div>
      ...
      {stats && selectedCourse ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', animation: 'fadeIn 0.5s' }}>
          ...
          {/* MIDDLE: Occupancy & Discourse */}
          <div style={{display:'grid', gridTemplateRows: 'auto 1fr', gap:'20px'}}>
             <div style={cardStyle}>
                <h3 style={{marginTop:0}}>Occupancy Breakdown</h3>
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'10px', textAlign:'center', marginTop:'15px'}}>
                    <div style={{background:'#e8f5e9', padding:'10px', borderRadius:'8px'}}><div style={{fontSize:'20px', fontWeight:'bold', color:'#2e7d32'}}>{stats.old_students}</div><div style={{fontSize:'12px'}}>Old</div></div>
                    <div style={{background:'#fff3e0', padding:'10px', borderRadius:'8px'}}><div style={{fontSize:'20px', fontWeight:'bold', color:'#ef6c00'}}>{stats.new_students}</div><div style={{fontSize:'12px'}}>New</div></div>
                    <div style={{background:'#e3f2fd', padding:'10px', borderRadius:'8px'}}><div style={{fontSize:'20px', fontWeight:'bold', color:'#1565c0'}}>{stats.servers}</div><div style={{fontSize:'12px'}}>Servers</div></div>
                </div>
             </div>
                    ...
                    <tbody>
                      {stats.languages.map((l, i) => (
                        <tr key={i} style={{borderBottom:'1px solid #f4f4f4', height:'30px'}}>
                          <td style={{padding:'5px 0'}}>{l.discourse_language}</td>
          ...
          {/* RIGHT: Applicant Types Bar Chart */}
          <div style={cardStyle}>
...
function GlobalAccommodationManager({ courses, onRoomClick }) {
  ...
  let maleFree = 0, maleOcc = 0;
  let femaleFree = 0, femaleOcc = 0;

  safeRooms.forEach(r => {
    const p = occupiedMap[normalize(r.room_no)];
    const isMaleRoom = r.gender_type === 'Male';
    if (p) {
      if (isMaleRoom) maleOcc++; else femaleOcc++;
    } else {
  ...
  const renderRoom = (room) => {
    ...
    // Color Logic
    let bgColor = '#e8f5e9'; // Free Green
    let borderColor = '#a5d6a7';
    if (isOccupied) {
        const isOld = occupant.conf_no && (occupant.conf_no.startsWith('O') || occupant.conf_no.startsWith('S'));
        if (isOld) {
            bgColor = '#e1bee7'; borderColor = '#8e24aa'; // Purple (Old)
        } else {
            bgColor = '#c8e6c9'; borderColor = '#2e7d32'; // Green (New)
        }
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{marginBottom:'20px'}}>
         <div style={{padding:'10px', background:'#e8f5e9', borderRadius:'8px', borderLeft:'5px solid #2e7d32'}}>
            <div style={{fontSize:'14px', fontWeight:'bold', color:'#2e7d32', marginBottom:'5px'}}>COURSE OCCUPANCY</div>
            <div style={{height:'100px'}}>
                <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={Object.entries(courseBreakdown).map(([name, count]) => ({ name: name.substring(0,15), count }))} layout="vertical" margin={{top: 5, right: 30, left: 20, bottom: 5}}>
                      <XAxis type="number" hide />
                      <YAxis dataKey="name" type="category" width={100} tick={{fontSize: 11}} />
                      <Tooltip />
                      <Bar dataKey="count" fill="#82ca9d" barSize={20} label={{ position: 'right', fontSize: 11 }} />
                   </BarChart>
                </ResponsiveContainer>
            </div>
         </div>
      ...
      <div style={{display:'flex', gap:'15px', marginBottom:'10px', fontSize:'12px', justifyContent:'center'}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'5px'}}><div style={{width:'12px', height:'12px', background:'#fff3e0', border:'1px solid #ffb74d'}}></div> Pending</div>
      </div>
...
// --- 4. MANAGE STUDENTS (Added View All Button + Modal) ---
function ParticipantList({ courses, refreshCourses }) {
  ...
  // View All Modal (Full Screen Table)
  if (viewAllMode) {
      return (
          <div style={{background:'white', padding:'20px', height:'100vh', overflow:'auto'}}>
             <div className="no-print" style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}>
                 <button onClick={() => setViewAllMode(false)} style={btnStyle(false)}>← Back</button>
                 <button onClick={() => window.print()} style={{...btnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Master List</button>
             </div>
             <h2 style={{textAlign:'center'}}>Master Student List - {selectedCourseName}</h2>
                 ...
                 <tbody>
                     {participants.map(p => (
                         <tr key={p.participant_id} style={{borderBottom:'1px solid #ddd'}}>
                             ...
                             <td style={{padding:'8px'}}>{p.conf_no}</td>
                             <td style={{padding:'8px'}}>{p.age}</td>
                             <td style={{padding:'8px'}}>{p.gender}</td>
                             <td style={{padding:'8px'}}>{p.phone_number}</td>
                             <td style={{padding:'8px'}}>{p.room_no}</td>
                             <td style={{padding:'8px'}}>{p.dining_seat_no} ({p.dining_seat_type})</td>
                             <td style={{padding:'8px'}}>{p.pagoda_cell_no}</td>
                             <td style={{padding:'8px'}}>{p.dhamma_hall_seat_no}</td>
