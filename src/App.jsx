...
function ParticipantList({ courses, refreshCourses }) {
  ...
  const prepareToken = (student) => {
      ...
      setPrintTokenData({ 
          ...
          cell: student.pagoda_cell_no||'-', room: student.room_no||'-' 
      });
      setTimeout(() => window.print(), 500);
  };
  ...
  const prepareBulkTokens = () => {
    ...
    setPrintBulkData(valid.sort((a,b)=>a.dhamma_hall_seat_no.localeCompare(b.dhamma_hall_seat_no, undefined, {numeric:true})).map(s=>({
        seat: s.dhamma_hall_seat_no, name: s.full_name, conf: s.conf_no, cell: s.pagoda_cell_no||'-', room: s.room_no||'-'
    })));
    setTimeout(()=>window.print(), 500);
  };
  ...
  if (viewMode === 'seating') {
      ...
      const Box = ({p, l}) => {
          // Parse courses_info to get "S" and "L" for display if possible, else default
          // Assuming format "S:3 L:1" or similar.
          let sVal = '-', lVal = '-';
          if (p && p.courses_info) {
          ...
          return (
              <div onClick={() => handleSeatClick(l, p)} style={{border: '2px solid black', background:'white', height:'100%', fontSize:'10px', display:'flex', flexDirection:'column', cursor:'pointer', position:'relative', minHeight:'90px'}}>
                  {/* HEADER: Seat No (Left) -- Room No (Right) */}
