...
// --- 1. GLOBAL ACCOMMODATION MANAGER (FIXED: Unmapped Rooms + Smart Match) ---
function GlobalAccommodationManager({ courses }) {
  ...
  // Normalizes room strings: "301-AI" -> "301AI", " 301 ai " -> "301AI"
  const normalize = (str) => str ? str.replace(/[\s-]+/g, '').toUpperCase() : '';
  
  const occupiedMap = {}; // Maps Normalized Room -> Participant
  const unmappedParticipants = []; // Stores students with invalid room numbers
  ...
  occupancy.forEach(p => { 
    if(p.room_no) {
      ...
      if (validRoomSet.has(normRoom)) {
        occupiedMap[normRoom] = p; 
      } else {
        unmappedParticipants.push(p); // Room doesn't exist in DB
      }
    }
  });
    ...
    <div style={cardStyle}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'15px', marginBottom:'20px'}}>
        ...
        <div style={{padding:'10px', background:'#e8f5e9', borderRadius:'6px'}}>
          <div style={{fontSize:'12px', color:'#2e7d32'}}>Valid Assignments</div>
          <div style={{fontSize:'20px', fontWeight:'bold', color:'#2e7d32'}}>{maleOcc + femaleOcc}</div>
        </div>
      </div>

      {/* UNMAPPED ROOMS WARNING (The Missing Link Fix) */}
      {unmappedParticipants.length > 0 && (
        <div style={{marginBottom:'20px', padding:'15px', background:'#fff3e0', borderRadius:'8px', border:'1px solid #ffcc80'}}>
          <h3 style={{margin:'0 0 10px 0', color:'#ef6c00'}}>⚠️ {unmappedParticipants.length} Students have Invalid/Missing Room Numbers</h3>
          <div style={{maxHeight:'100px', overflowY:'auto', fontSize:'12px'}}>
             <table style={{width:'100%'}}>
               ...
               <tbody>
                 {unmappedParticipants.map((p, i) => (
                   <tr key={i}>
                     <td>{p.full_name} ({p.conf_no})</td>
                     <td style={{fontWeight:'bold', color:'red'}}>{p.room_no}</td>
                     <td><button onClick={() => setEditingRoom({ p, newRoomNo: '' })} style={{cursor:'pointer', background:'white', border:'1px solid #ccc', borderRadius:'4px'}}>Fix Room</button></td>
                   </tr>
                 ))}
               </tbody>
             </table>
