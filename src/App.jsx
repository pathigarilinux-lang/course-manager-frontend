...
// --- 4. MANAGE STUDENTS (Fixed: Crash-Proof, Safe Swap, Auto-Assign) ---
function ParticipantList({ courses, refreshCourses }) {
  ...
  // --- HELPERS (Defined at top to avoid errors) ---
  ...
  const handleAutoAssign = async () => {
    ...
    setAssignProgress(''); alert(`✅ Assignment Complete! Assigned ${updates.length} students.`); loadStudents();
  };
  ...
  if (viewMode === 'seating') { 
    ...
    const SeatBox = ({ p, label }) => { const isSelected = selectedSeat && selectedSeat.label === label; const isOld = p && getCategoryRank(p.conf_no) === 0; return ( <div onClick={() => handleSeatClick(label, p)} style={{border: isSelected ? '3px solid gold' : '1px solid #ccc', background: p ? (isOld ? '#fff9c4' : 'white') : '#f0f0f0', height:'60px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontSize:'10px', cursor:'pointer'}}> {p ? (<><div style={{fontWeight:'bold', fontSize:'13px', color: '#007bff'}}>{p.dhamma_hall_seat_no}</div><div style={{overflow:'hidden', textOverflow:'ellipsis', width:'90%', whiteSpace:'nowrap'}}>{(p.full_name||'').split(' ')[0]}</div><div style={{fontSize:'9px'}}>{p.conf_no}</div></>) : <span style={{color:'#ccc'}}>{label}</span>} </div> ); };
