...
function App() {
  ...
  const handleLogin = (userData) => {
      ...
      // ✅ REDIRECT: If Gate user, go straight to Gate tab
  ...
  const NavItem = ({ id, label, icon: Icon, roles = [] }) => {
      ...
      // 2. ✅ SPECIAL RESTRICTION FOR 'gate' ROLE
      // If user is 'gate', they can ONLY see 'gate' (and 'seva'). HIDE EVERYTHING ELSE.
    ...
    <div style={styles.dashboardContainer}>
      ...
      <aside style={{
          ...
          <div style={{ padding: '20px', textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)', marginBottom: '10px' }}>
              ...
              {isSidebarOpen && <div style={{ fontSize: '12px', opacity: 0.7, marginTop: '5px' }}>{user.role === 'gate' ? 'Gate Access' : (user.role === 'dn1ops' ? 'DN1 Operations' : 'Management System')}</div>}
          </div>

          <nav style={{ flex: 1, overflowY: 'auto' }}>
              <NavItem id="dashboard" label="Dashboard" icon={LayoutDashboard} />
              
              {/* ✅ UPDATED: Enabled for 'gate' and 'dn1ops' */}
              <NavItem id="gate" label="Gate / Reception" icon={UserCheck} roles={['admin', 'staff', 'dn1ops', 'gate']} />
              
              <NavItem id="checkin" label="Check-In" icon={UserPlus} />
              <NavItem id="students" label="Participants" icon={Users} />
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden', background: '#f4f6f9' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '30px' }}>
              ...
              {/* ✅ UPDATED: Pass userRole to GateReception */}
