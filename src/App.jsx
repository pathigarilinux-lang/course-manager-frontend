        ...
        <div style={{background:'white', padding:'20px', borderRadius:'10px', width:'80%', maxHeight:'80vh', overflowY:'auto'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(60px, 1fr))', gap:'10px'}}>
                {options.map(opt => {
                    ...
                    return (<button key={opt} type="button" onClick={() => !isOcc && onSelect(opt)} disabled={isOcc} style={{padding:'10px', borderRadius:'5px', border:'none', cursor: isOcc?'not-allowed':'pointer', background: isOcc ? '#ffcdd2' : (String(selected)===String(opt) ? '#007bff' : '#c8e6c9'), color: (String(selected)===String(opt) ? 'white' : 'black'), fontWeight:'bold'}}>{opt}</button>);
                })}
            </div>
        </div>
    </div>
