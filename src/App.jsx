...
// --- 1. MANAGE STUDENTS (With Reset, Delete, Dhamma Seat) ---
function ParticipantList({ courses, refreshCourses }) {
  const [courseId, setCourseId] = useState('');
  const [participants, setParticipants] = useState([]);
  const [search, setSearch] = useState('');
  const [editingStudent, setEditingStudent] = useState(null);
  const [showDiningSheet, setShowDiningSheet] = useState(false);
  ...
  const handleDeleteStudent = async (id) => { if (window.confirm("Delete student?")) { await fetch(`${API_URL}/participants/${id}`, { method: 'DELETE' }); loadStudents(); } };
  const handleResetCourse = async () => { if (window.confirm("⚠️ WARNING: This will DELETE ALL STUDENTS and EXPENSES for this course. The course name will remain. Are you sure?")) { await fetch(`${API_URL}/courses/${courseId}/reset`, { method: 'DELETE' }); loadStudents(); alert("Course Data Reset."); } };
  const handleDeleteCourse = async () => { if (window.confirm("🛑 DANGER: This will delete the ENTIRE COURSE, including all students and history. This cannot be undone.")) { await fetch(`${API_URL}/courses/${courseId}`, { method: 'DELETE' }); refreshCourses(); setCourseId(''); alert("Course Deleted."); } };
  ...
  const handleEditSave = async (e) => {
    ...
    setEditingStudent(null); loadStudents();
  };
  ...
  // --- SEATING PLAN VISUAL ---
  if (showSeatingPlan) {
    const sortedSeats = participants.filter(p => p.dhamma_hall_seat_no).sort((a,b) => (a.dhamma_hall_seat_no || 'Z').localeCompare(b.dhamma_hall_seat_no || 'Z'));
    return (
      <div style={cardStyle}>
        <div className="no-print" style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}><button onClick={() => setShowSeatingPlan(false)} style={btnStyle(false)}>← Back</button><button onClick={() => window.print()} style={{...btnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Plan</button></div>
        <div style={{textAlign:'center'}}><h1>Dhamma Hall Seating Plan</h1><h3>{selectedCourseName}</h3></div>
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(150px, 1fr))', gap:'10px', marginTop:'20px'}}>
          {sortedSeats.map(p => (
            <div key={p.participant_id} style={{border:'1px solid #333', padding:'10px', textAlign:'center', borderRadius:'5px', background:'#f9f9f9'}}>
              <div style={{fontWeight:'bold', fontSize:'18px', marginBottom:'5px'}}>{p.dhamma_hall_seat_no}</div>
  ...
  // --- DINING SHEET ---
  if (showDiningSheet) {
    const sortedDining = [...participants].sort((a,b) => (a.dining_seat_no || 'Z').localeCompare(b.dining_seat_no || 'Z'));
    return (
      <div style={cardStyle}>
        <div className="no-print" style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}><button onClick={() => setShowDiningSheet(false)} style={btnStyle(false)}>← Back</button><button onClick={() => window.print()} style={{...btnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Sheet</button></div>
        <div style={{textAlign:'center'}}><h1>Dining Seating Chart</h1><h3>{selectedCourseName}</h3></div>
        <table style={{width:'100%', borderCollapse:'collapse', fontSize:'16px'}}><thead><tr style={{borderBottom:'2px solid black'}}><th style={thPrint}>Seat</th><th style={thPrint}>Name</th><th style={thPrint}>Room</th><th style={thPrint}>Pagoda</th><th style={thPrint}>Lang</th></tr></thead><tbody>{sortedDining.filter(p=>p.status==='Arrived').map(p=>(<tr key={p.participant_id} style={{borderBottom:'1px solid #ddd'}}><td style={{padding:'12px', fontWeight:'bold'}}>{p.dining_seat_no}</td><td style={{padding:'12px'}}>{p.full_name}</td><td style={{padding:'12px'}}>{p.room_no}</td><td style={{padding:'12px'}}>{p.pagoda_cell_no}</td><td style={{padding:'12px'}}>{p.discourse_language}</td></tr>))}</tbody></table>
      </div>
    );
  }
    ...
    <div style={cardStyle}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:'20px', flexWrap:'wrap', gap:'10px'}}>
        ...
        <div style={{display:'flex', gap:'5px'}}>
          <button onClick={() => setShowDiningSheet(true)} disabled={!courseId} style={{...btnStyle(false), fontSize:'12px'}}>🍽️ Dining List</button>
          <button onClick={() => setShowSeatingPlan(true)} disabled={!courseId} style={{...btnStyle(false), fontSize:'12px'}}>🧘 Dhamma Plan</button>
        </div>
      </div>
      
      {/* DANGER ZONE */}
      {courseId && (
        <div style={{background:'#fff5f5', border:'1px solid #feb2b2', padding:'10px', borderRadius:'5px', marginBottom:'20px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          <span style={{color:'#c53030', fontWeight:'bold', fontSize:'13px'}}>⚠️ Course Admin Area:</span>
          <div>
            <button onClick={handleResetCourse} style={{background:'#e53e3e', color:'white', border:'none', padding:'5px 10px', borderRadius:'4px', cursor:'pointer', marginRight:'10px', fontSize:'12px'}}>Reset Data</button>
            <button onClick={handleDeleteCourse} style={{background:'red', color:'white', border:'none', padding:'5px 10px', borderRadius:'4px', cursor:'pointer', fontSize:'12px'}}>Delete Course</button>
          </div>
        </div>
      )}
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{width:'100%', borderCollapse:'collapse', fontSize:'14px'}}>
          <thead><tr style={{background:'#f1f1f1', textAlign:'left'}}><th style={tdStyle}>Name</th><th style={tdStyle}>Conf No</th><th style={tdStyle}>Dining</th><th style={tdStyle}>Dhamma</th><th style={tdStyle}>Room</th><th style={tdStyle}>Pagoda</th><th style={tdStyle}>Actions</th></tr></thead>
          <tbody>
            {filtered.map(p => (
              <tr key={p.participant_id} style={{borderBottom:'1px solid #eee'}}>
                ...
                <td style={tdStyle}>{p.dhamma_hall_seat_no||'-'}</td>
                <td style={tdStyle}>{p.room_no||'-'}</td>
                <td style={tdStyle}>{p.pagoda_cell_no||'-'}</td>
                <td style={tdStyle}><button onClick={() => setEditingStudent(p)} style={{marginRight:'5px', cursor:'pointer'}}>✏️</button><button onClick={() => handleDeleteStudent(p.participant_id)} style={{color:'red', cursor:'pointer'}}>🗑️</button></td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <div style={{background:'white', padding:'30px', borderRadius:'10px', width:'400px'}}>
            ...
            <form onSubmit={handleEditSave} style={{display:'flex', flexDirection:'column', gap:'10px'}}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
                ...
                <div><label>Dhamma Seat</label><input style={inputStyle} value={editingStudent.dhamma_hall_seat_no||''} onChange={e => setEditingStudent({...editingStudent, dhamma_hall_seat_no: e.target.value})} /></div>
              </div>
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
                ...
                <div><label>Pagoda</label><input style={inputStyle} value={editingStudent.pagoda_cell_no||''} onChange={e => setEditingStudent({...editingStudent, pagoda_cell_no: e.target.value})} /></div>
              </div>
...
// --- 2. CHECK-IN FORM (With Dhamma Seat) ---
function CheckInForm({ courses }) {
  const [participants, setParticipants] = useState([]);
  const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', language: 'English', pagodaCell: '', laptop: '', confNo: '', dhammaSeat: '' });
    ...
    <div style={cardStyle}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '800px' }}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr', gap:'10px'}}>
          ...
          <div><label style={labelStyle}>Dhamma Seat</label><input style={inputStyle} value={formData.dhammaSeat} onChange={e => setFormData({...formData, dhammaSeat: e.target.value})} /></div>
