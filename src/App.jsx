...
// --- 1. ZERO DAY DASHBOARD (UPDATED VISUALS & LABELS) ---
    ...
    <div>
      ...
      {stats && selectedCourse ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', animation: 'fadeIn 0.5s' }}>
          ...
          {/* MIDDLE: Discourse Count */}
          <div style={cardStyle}>
             <h3 style={{marginTop:0}}>Discourse Language</h3>
             {stats.languages && stats.languages.length > 0 ? (
               <div style={{maxHeight:'250px', overflowY:'auto'}}>
               <table style={{width:'100%', fontSize:'13px', borderCollapse:'collapse'}}>
                 <thead><tr style={{textAlign:'left', borderBottom:'1px solid #eee', color:'#666'}}><th>Lang</th><th style={{width:'30px'}}>M</th><th style={{width:'30px'}}>F</th><th style={{textAlign:'right'}}>Tot</th></tr></thead>
                 <tbody>
                   {stats.languages.map((l, i) => (
                     <tr key={i} style={{borderBottom:'1px solid #f4f4f4', height:'30px'}}>
                       <td style={{fontWeight:'500'}}>{l.discourse_language}</td>
                       <td style={{color:'#007bff', fontWeight:'bold'}}>{l.male_count}</td>
                       <td style={{color:'#e91e63', fontWeight:'bold'}}>{l.female_count}</td>
                       <td style={{textAlign:'right', fontWeight:'bold'}}>{l.total}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
               </div>
             ) : <p style={{color:'#888', textAlign:'center'}}>No students checked in yet.</p>}
          </div>

          {/* RIGHT: Live Counts (Bar Chart) */}
          <div style={cardStyle}>
            <h3 style={{marginTop:0}}>Live Counts (M/F)</h3>
            <div style={{height:'250px', width:'100%'}}>
               <ResponsiveContainer width="100%" height="100%">
...
// --- 2. GLOBAL ACCOMMODATION MANAGER (UPDATED: Course Bar + Colors) ---
function GlobalAccommodationManager({ courses, onRoomClick }) {
  ...
  const occupiedMap = {}; const courseCounts = {};
  occupancy.forEach(p => { 
    if(p.room_no) {
  ...
  // Prepare Chart Data for Course Occupancy
  const courseData = Object.keys(courseCounts).map(name => ({ name: name.substring(0,15), count: courseCounts[name] }));
  ...
  safeRooms.forEach(r => {
    ...
    if (p) { if(isMale) maleOcc++; else femaleOcc++; } 
    else { if(isMale) maleFree++; else femaleFree++; }
  });
  ...
  const renderRoom = (room) => {
    ...
    // COLOR LOGIC: Old vs New
    let bgColor = '#e8f5e9'; // Default Free Green
    let borderColor = '#a5d6a7';

    if (isOccupied) {
       const isOld = occupant.conf_no && (occupant.conf_no.startsWith('O') || occupant.conf_no.startsWith('S'));
       if (isOld) {
          bgColor = '#e1bee7'; // Purple for Old
          borderColor = '#8e24aa';
       } else {
          bgColor = '#c8e6c9'; // Green for New
          borderColor = '#2e7d32';
       }
    }
      ...
      >
        ...
        {isOccupied ? (
          <div style={{fontSize:'11px', color: '#333', marginTop:'4px'}}>
            ...
            <div style={{fontSize:'8px', background: 'rgba(255,255,255,0.6)', borderRadius:'4px', marginTop:'2px'}}>🔄 Swap</div>
          </div>
        ) : (
            <div>
                <div style={{fontSize:'9px', color:'#2e7d32', marginTop:'4px'}}>FREE</div>
    ...
    <div style={cardStyle}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 2fr', gap:'15px', marginBottom:'20px'}}>
        <div style={{padding:'12px', background:'#e3f2fd', borderRadius:'8px', borderLeft:'5px solid #1565c0'}}>
          <div style={{fontSize:'14px', fontWeight:'bold', color:'#1565c0'}}>MALE WING</div>
          <div>Occ: <strong>{maleOcc}</strong> / Free: <strong>{maleFree}</strong></div>
        </div>
        <div style={{padding:'12px', background:'#fce4ec', borderRadius:'8px', borderLeft:'5px solid #ad1457'}}>
          <div style={{fontSize:'14px', fontWeight:'bold', color:'#ad1457'}}>FEMALE WING</div>
          <div>Occ: <strong>{femaleOcc}</strong> / Free: <strong>{femaleFree}</strong></div>
        </div>
        {/* Course Occupancy Bar Chart */}
        <div style={{background:'white', borderRadius:'8px', border:'1px solid #eee', padding:'5px', height:'100px'}}>
             <ResponsiveContainer width="100%" height="100%">
               <BarChart data={courseData} layout="vertical" margin={{top: 5, right: 30, left: 20, bottom: 5}}>
                 <XAxis type="number" hide />
                 <YAxis dataKey="name" type="category" width={80} tick={{fontSize: 10}} />
                 <Tooltip />
                 <Bar dataKey="count" fill="#82ca9d" barSize={15} label={{ position: 'right', fontSize: 10 }} />
               </BarChart>
             </ResponsiveContainer>
        </div>
      </div>
      ...
      {/* Legend */}
      <div style={{display:'flex', gap:'15px', marginBottom:'10px', fontSize:'12px', justifyContent:'center'}}>
        <div style={{display:'flex', alignItems:'center', gap:'5px'}}><div style={{width:'12px', height:'12px', background:'#e1bee7', border:'1px solid #8e24aa'}}></div> Old Student</div>
        <div style={{display:'flex', alignItems:'center', gap:'5px'}}><div style={{width:'12px', height:'12px', background:'#c8e6c9', border:'1px solid #2e7d32'}}></div> New Student</div>
        <div style={{display:'flex', alignItems:'center', gap:'5px'}}><div style={{width:'12px', height:'12px', background:'#e8f5e9', border:'1px solid #a5d6a7'}}></div> Empty</div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
         <div style={{border:'1px solid #90caf9', borderRadius:'8px', padding:'10px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'8px'}}>{maleRooms.map(renderRoom)}</div>
         </div>
         <div style={{border:'1px solid #f48fb1', borderRadius:'8px', padding:'10px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'8px'}}>{femaleRooms.map(renderRoom)}</div>
         </div>
      </div>
...
// --- 3. STUDENT FORM (UPDATED: Restricted Inputs) ---
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  const [participants, setParticipants] = useState([]);
  const [rooms, setRooms] = useState([]);
  const [occupancy, setOccupancy] = useState([]);
  const [selectedStudent, setSelectedStudent] = useState(null);
  const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', language: 'English', pagodaCell: '', laptop: '', confNo: '', dhammaSeat: '', specialSeating: '', seatType: 'Chair' });
    ...
    <div style={cardStyle}>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '900px' }}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'20px'}}>
           ...
           <div><label style={labelStyle}>🛏️ Room No</label><select style={{...inputStyle, background: preSelectedRoom ? '#e8f5e9' : 'white'}} value={formData.roomNo} onChange={e => setFormData({...formData, roomNo: e.target.value})} required><option value="">-- Select Free Room --</option>{preSelectedRoom && <option value={preSelectedRoom}>{preSelectedRoom} (Selected)</option>}{availableRooms.map(r => <option key={r.room_id} value={r.room_no}>{r.room_no}</option>)}</select></div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:'20px'}}>
           <div><label style={labelStyle}>📱 Mobile Locker</label><select style={inputStyle} value={formData.mobileLocker} onChange={e => setFormData({...formData, mobileLocker: e.target.value})}><option value="">None</option>{NUMBER_OPTIONS.map(n=><option key={n} value={n}>{n}</option>)}</select></div>
           <div><label style={labelStyle}>💍 Val Locker</label><select style={inputStyle} value={formData.valuablesLocker} onChange={e => setFormData({...formData, valuablesLocker: e.target.value})}><option value="">None</option>{NUMBER_OPTIONS.map(n=><option key={n} value={n}>{n}</option>)}</select></div>
           <div><label style={labelStyle}>🧺 Laundry Token</label><select style={inputStyle} value={formData.laundryToken} onChange={e => setFormData({...formData, laundryToken: e.target.value})}><option value="">None</option>{NUMBER_OPTIONS.map(n=><option key={n} value={n}>{n}</option>)}</select></div>
...
function ParticipantList({ courses, refreshCourses }) {
  ...
  if (viewMode === 'seating') { 
    // Smart Grid Logic: Old(Seniority) then New(Age)
    ...
    const males = sortGroup(participants.filter(p => p.gender==='Male' && p.status==='Arrived'));
    const females = sortGroup(participants.filter(p => p.gender==='Female' && p.status==='Arrived'));

    const SeatBox = ({ p, label }) => {
        const isOld = p && p.conf_no && (p.conf_no.startsWith('O') || p.conf_no.startsWith('S'));
        return (
          <div style={{border:'1px solid #ccc', background: p ? (isOld ? '#fff9c4' : 'white') : '#f0f0f0', height:'60px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontSize:'10px', textAlign:'center'}}>
            {p ? (<><div style={{fontWeight:'bold', fontSize:'13px', color:'#007bff'}}>{label}</div><div style={{overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', width:'90%'}}>{(p.full_name || '').substring(0, 10)}</div><div style={{fontSize:'9px'}}>Conf: {p.conf_no}</div><div style={{fontSize:'8px', color:'#666'}}>Pag: {p.pagoda_cell_no || '-'} | Din: {p.dining_seat_no || '-'}</div></>) : <span style={{color:'#ccc'}}>{label}</span>}
          </div>
        );
    };
    
    const renderGrid = (students, cols, rows) => {
      let grid = [];
      for (let r = 0; r < rows; r++) {
        ...
        for (let c = 0; c < cols; c++) {
           ...
           rowCells.push(<SeatBox key={seatLabel} p={students[idx]} label={seatLabel} />);
        }
...
// --- 5. STORE & FINANCE (Updated: Invoice & Summary UI) ---
function ExpenseTracker({ courses }) {
  const [courseId, setCourseId] = useState(''); const [participants, setParticipants] = useState([]); const [selectedStudentId, setSelectedStudentId] = useState(''); const [studentToken, setStudentToken] = useState(''); const [expenseType, setExpenseType] = useState('Laundry Token'); const [amount, setAmount] = useState(''); const [history, setHistory] = useState([]); const [status, setStatus] = useState(''); const [reportMode, setReportMode] = useState(''); const [financialData, setFinancialData] = useState([]); const [editingId, setEditingId] = useState(null);
    ...
    <div style={cardStyle}>
      ...
      <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:'15px'}}>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px'}}>
          <select style={inputStyle} onChange={e => setCourseId(e.target.value)} required> <option value="">-- Select Course --</option> {courses.map(c => <option key={c.course_id} value={c.course_id}>{c.course_name}</option>)} </select>
          <select style={inputStyle} onChange={e => setSelectedStudentId(e.target.value)} disabled={!courseId} required> <option value="">-- Select Student --</option> {participants.map(p => <option key={p.participant_id} value={p.participant_id}>{p.full_name}</option>)} </select>
        </div>
        ...
        <button type="submit" style={{...btnStyle(true), background: editingId ? '#ffc107' : '#28a745', color: editingId ? 'black' : 'white'}}> {editingId ? 'Update Record' : 'Save Record'} </button>
      </form>
      <div style={{marginTop:'30px', borderTop:'1px solid #eee', paddingTop:'15px'}}>
         <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px'}}>
           <h3 style={{margin:0, color:'#555'}}>History</h3>
           <div style={{display:'flex', gap:'5px'}}>
             <button onClick={() => setReportMode('invoice')} disabled={!selectedStudentId} style={{...quickBtnStyle(!!selectedStudentId), background: selectedStudentId ? '#17a2b8' : '#e2e6ea'}}>🖨️ Invoice</button>
             <button onClick={loadFinancialReport} disabled={!courseId} style={{...quickBtnStyle(!!courseId), background: courseId ? '#ff9800' : '#e2e6ea'}}>💰 Summary</button>
           </div>
         </div>
         {history.length === 0 ? ( <p style={{color:'#888', fontSize:'13px'}}>No expenses recorded.</p> ) : ( <div style={{maxHeight:'200px', overflowY:'auto'}}><table style={{width:'100%', fontSize:'13px', borderCollapse:'collapse'}}><thead><tr style={{textAlign:'left', borderBottom:'1px solid #eee'}}><th>Item</th><th>Date</th><th>₹</th><th></th></tr></thead><tbody>{history.map(h => (<tr key={h.expense_id} style={{borderBottom:'1px solid #eee'}}><td style={{padding:'5px'}}>{h.expense_type}</td><td style={{padding:'5px', color:'#666'}}>{new Date(h.recorded_at).toLocaleDateString()}</td><td style={{padding:'5px', fontWeight:'bold'}}>₹{h.amount}</td><td style={{textAlign:'right'}}><button onClick={()=>handleEditClick(h)} style={{marginRight:'5px', cursor:'pointer'}}>✏️</button><button onClick={()=>handleDeleteExpense(h.expense_id)} style={{color:'red', cursor:'pointer'}}>🗑️</button></td></tr>))}</tbody></table><div style={{textAlign:'right', marginTop:'10px', fontWeight:'bold', color:'#2e7d32'}}>Total Due: ₹{totalDue}</div></div> )}
      </div>
    </div>
  );
}

// --- 6. COURSE ADMIN & STYLES (Unchanged) ---
function CourseAdmin({ courses, refreshCourses, setView }) {
  const [activeTab, setActiveTab] = useState('create');
  return ( <div style={cardStyle}> <div style={{display:'flex', borderBottom:'1px solid #ddd', marginBottom:'20px', gap:'10px'}}> <button onClick={()=>setActiveTab('create')} style={{padding:'10px', background:activeTab==='create'?'#eee':'white', border:'none', borderBottom:activeTab==='create'?'2px solid #007bff':'none', cursor:'pointer'}}>➕ New Course</button> <button onClick={()=>setActiveTab('upload')} style={{padding:'10px', background:activeTab==='upload'?'#eee':'white', border:'none', borderBottom:activeTab==='upload'?'2px solid #007bff':'none', cursor:'pointer'}}>📂 Upload CSV</button> <button onClick={()=>setActiveTab('manual')} style={{padding:'10px', background:activeTab==='manual'?'#eee':'white', border:'none', borderBottom:activeTab==='manual'?'2px solid #007bff':'none', cursor:'pointer'}}>✍️ Manual Entry</button> </div> {activeTab === 'create' && <CreateCourseForm refreshCourses={refreshCourses} setView={setView} />} {activeTab === 'upload' && <UploadParticipants courses={courses} setView={setView} />} {activeTab === 'manual' && <ManualStudentForm courses={courses} setView={setView} />} </div> );
}
