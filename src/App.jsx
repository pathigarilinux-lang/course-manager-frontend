...
function Dashboard({ courses }) {
  ...
  const arrivalData = stats ? [ { name: 'Arrived', Male: stats.arrived_m || 0, Female: stats.arrived_f || 0 }, { name: 'Pending', Male: stats.pending_m || 0, Female: stats.pending_f || 0 }, { name: 'Cancelled', Male: stats.cancelled_m || 0, Female: stats.cancelled_f || 0 } ] : [];
    ...
    <div>
      ...
      {stats && selectedCourse ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', animation: 'fadeIn 0.5s' }}>
          <div style={cardStyle}><h3 style={{marginTop:0}}>Status Overview (M/F)</h3><div style={{height:'250px', width:'100%'}}><ResponsiveContainer width="100%" height="100%"><BarChart data={arrivalData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Legend /><Bar dataKey="Male" fill="#007bff" stackId="a" /><Bar dataKey="Female" fill="#e91e63" stackId="a" /></BarChart></ResponsiveContainer></div></div>
          
          <div style={{display:'grid', gridTemplateRows: 'auto 1fr', gap:'20px'}}>
             <div style={cardStyle}><h3 style={{marginTop:0}}>Occupancy Breakdown</h3><div style={{display:'flex', justifyContent:'space-around', alignItems:'center', height:'120px'}}><ResponsiveContainer width="40%" height="100%"><PieChart><Pie data={occData} cx="50%" cy="50%" innerRadius={30} outerRadius={50} dataKey="value" label>{occData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer><div style={{fontSize:'12px'}}><div style={{color:'#2e7d32'}}>Old: <strong>{stats.old_students}</strong></div><div style={{color:'#ef6c00'}}>New: <strong>{stats.new_students}</strong></div><div style={{color:'#1565c0'}}>Servers: <strong>{stats.servers}</strong></div></div></div></div>
             <div style={cardStyle}><h3 style={{marginTop:0}}>Discourse Count</h3>{stats.languages && stats.languages.length > 0 ? (<div style={{maxHeight:'150px', overflowY:'auto'}}><table style={{width:'100%', fontSize:'13px'}}><thead><tr style={{textAlign:'left', borderBottom:'1px solid #eee'}}><th>Lang</th><th>M</th><th>F</th><th style={{textAlign:'right'}}>Total</th></tr></thead><tbody>{stats.languages.map((l, i) => (<tr key={i} style={{borderBottom:'1px solid #f4f4f4'}}><td style={{padding:'8px 0'}}>{l.discourse_language || 'Unknown'}</td><td style={{padding:'8px 0', color:'#007bff'}}>{l.male_count}</td><td style={{padding:'8px 0', color:'#e91e63'}}>{l.female_count}</td><td style={{padding:'8px 0', textAlign:'right', fontWeight:'bold'}}>{l.total}</td></tr>))}</tbody></table></div>) : <p style={{color:'#888'}}>No data.</p>}</div>
          </div>

          <div style={cardStyle}><h3 style={{marginTop:0}}>Live Counts (M/F)</h3><div style={{height:'250px', width:'100%'}}><ResponsiveContainer width="100%" height="100%"><BarChart data={typeData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Legend /><Bar dataKey="Male" fill="#007bff" stackId="a" /><Bar dataKey="Female" fill="#e91e63" stackId="a" /></BarChart></ResponsiveContainer></div></div>
        </div>
...
function GlobalAccommodationManager({ courses, onRoomClick }) {
  ...
  const occupiedMap = {}; const courseBreakdown = {}; const unmappedParticipants = [];
  const safeRooms = rooms || []; const validRoomSet = new Set(safeRooms.map(r => normalize(r.room_no)));

  (occupancy || []).forEach(p => { if(p.room_no) { const normRoom = normalize(p.room_no); if (validRoomSet.has(normRoom)) { occupiedMap[normRoom] = p; const cName = p.course_name || 'Unknown'; courseBreakdown[cName] = (courseBreakdown[cName] || 0) + 1; } else unmappedParticipants.push(p); } });
  const maleRooms = safeRooms.filter(r => r.gender_type === 'Male'); const femaleRooms = safeRooms.filter(r => r.gender_type === 'Female');
  let maleFree = 0, maleOcc = 0, maleOld = 0, maleNew = 0; let femaleFree = 0, femaleOcc = 0, femaleOld = 0, femaleNew = 0;

  safeRooms.forEach(r => { const p = occupiedMap[normalize(r.room_no)]; const isMaleRoom = r.gender_type === 'Male'; if (p) { const isOld = p.conf_no && (p.conf_no.startsWith('O') || p.conf_no.startsWith('S')); if (isMaleRoom) { maleOcc++; if(isOld) maleOld++; else maleNew++; } else { femaleOcc++; if(isOld) femaleOld++; else femaleNew++; } } else { if (isMaleRoom) maleFree++; else femaleFree++; } });

  const renderRoom = (room) => { const occupant = occupiedMap[normalize(room.room_no)]; const isOccupied = !!occupant; const isArrived = isOccupied && occupant.status === 'Arrived';
    ...
    <div style={cardStyle}>
      <div className="no-print" style={{display:'flex', justifyContent:'space-between', marginBottom:'20px', alignItems:'center', flexWrap:'wrap', gap:'10px'}}>
        ...
        <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
           <div style={{display:'flex', gap:'5px', alignItems:'center', background:'#f9f9f9', padding:'5px', borderRadius:'5px', border:'1px solid #eee'}}> <input style={{...inputStyle, width:'60px', padding:'5px'}} placeholder="No" value={newRoom.roomNo} onChange={e=>setNewRoom({...newRoom, roomNo:e.target.value})} /> <select style={{...inputStyle, width:'70px', padding:'5px'}} value={newRoom.type} onChange={e=>setNewRoom({...newRoom, type:e.target.value})}><option>Male</option><option>Female</option></select> <button onClick={handleAddRoom} style={{...quickBtnStyle(true), background:'#007bff', color:'white', padding:'5px 10px', fontSize:'11px'}}>+ Add</button> </div>
           <button onClick={loadData} style={{...btnStyle(false), fontSize:'12px'}}>↻ Refresh</button> <button onClick={() => window.print()} style={{...quickBtnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Status</button>
        </div>
      </div>
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'15px', marginBottom:'20px'}}>
        <div style={{padding:'12px', background:'#e3f2fd', borderRadius:'8px', borderLeft:'5px solid #1565c0'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#1565c0', marginBottom:'5px'}}>MALE WING</div> <div style={{display:'flex', justifyContent:'space-between', fontSize:'13px'}}><span>Occupied: <strong>{maleOcc}</strong></span><span>Free: <strong>{maleFree}</strong></span></div> <div style={{fontSize:'11px', color:'#555', marginTop:'4px'}}>Old: <strong>{maleOld}</strong> | New: <strong>{maleNew}</strong></div> </div>
        <div style={{padding:'12px', background:'#fce4ec', borderRadius:'8px', borderLeft:'5px solid #ad1457'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#ad1457', marginBottom:'5px'}}>FEMALE WING</div> <div style={{display:'flex', justifyContent:'space-between', fontSize:'13px'}}><span>Occupied: <strong>{femaleOcc}</strong></span><span>Free: <strong>{femaleFree}</strong></span></div> <div style={{fontSize:'11px', color:'#555', marginTop:'4px'}}>Old: <strong>{femaleOld}</strong> | New: <strong>{femaleNew}</strong></div> </div>
        <div style={{padding:'12px', background:'#e8f5e9', borderRadius:'8px', borderLeft:'5px solid #2e7d32'}}> <div style={{fontSize:'14px', fontWeight:'bold', color:'#2e7d32', marginBottom:'5px'}}>COURSE OCCUPANCY</div> <div style={{fontSize:'11px', color:'#333', marginTop:'5px', display:'flex', flexWrap:'wrap', gap:'10px'}}> {Object.entries(courseBreakdown).length > 0 ? Object.entries(courseBreakdown).map(([name, count]) => (<span key={name} style={{background:'white', padding:'2px 6px', borderRadius:'4px', border:'1px solid #ccc'}}>{name.substring(0,10)}.. : <strong>{count}</strong></span>)) : "No occupancy"} </div> </div>
      </div>
...
// --- 3. ONBOARDING FORM (Smart Room Filtering + Section UI) ---
function OnboardingForm({ courses, preSelectedRoom, clearRoom }) {
  const [participants, setParticipants] = useState([]);
  const [rooms, setRooms] = useState([]);
  const [occupancy, setOccupancy] = useState([]); 
  const [selectedStudent, setSelectedStudent] = useState(null);
  const [formData, setFormData] = useState({ courseId: '', participantId: '', roomNo: '', seatNo: '', laundryToken: '', mobileLocker: '', valuablesLocker: '', language: 'English', pagodaCell: '', laptop: '', confNo: '', dhammaSeat: '', specialSeating: '', seatType: 'Floor' });
...
// --- 4. MANAGE STUDENTS (With Smart Dhamma Hall Grid & View) ---
function ParticipantList({ courses, refreshCourses }) {
  ...
  if (viewMode === 'seating') { 
    ...
    const SeatBox = ({ p, label }) => (<div style={{border:'1px solid #ccc', background: p ? 'white' : '#f0f0f0', height:'60px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontSize:'10px', textAlign:'center'}}>{p ? (<><div style={{fontWeight:'bold', fontSize:'13px', color:'#007bff'}}>{label}</div><div style={{overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', width:'90%'}}>{p.full_name.split(' ')[0]}</div><div style={{color:'#666'}}>{p.conf_no}</div></>) : <span style={{color:'#ccc'}}>{label}</span>}</div>);
    
    // MALE: 10 Cols, 8 Rows | FEMALE: 7 Cols, 7 Rows
    ...
    return ( <div style={cardStyle}> <div className="no-print" style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}><button onClick={() => setViewMode('list')} style={btnStyle(false)}>← Back</button><button onClick={() => window.print()} style={{...btnStyle(true), background:'#28a745', color:'white'}}>🖨️ Print Plan</button></div><div style={{textAlign:'center', marginBottom:'20px'}}><h1>Dhamma Hall Seating</h1><h3>{selectedCourseName}</h3></div><div style={{display:'flex', gap:'40px'}}><div style={{flex:'0 0 40%'}}><h3 style={{textAlign:'center', background:'#fce4ec', padding:'5px'}}>FEMALE (7x7)</h3>{renderGrid(females, 7, 7)}</div><div style={{flex:'0 0 5%', display:'flex', alignItems:'center', justifyContent:'center', borderLeft:'1px dashed #ccc', borderRight:'1px dashed #ccc'}}><div style={{writingMode:'vertical-rl', textOrientation:'upright', letterSpacing:'5px', color:'#ccc', fontWeight:'bold'}}>AISLE</div></div><div style={{flex:'0 0 55%'}}><h3 style={{textAlign:'center', background:'#e3f2fd', padding:'5px'}}>MALE (10x8)</h3>{renderGrid(males, 10, 8)}</div></div></div> );
  }
