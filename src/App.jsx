...
function Dashboard({ courses }) {
  ...
  return (
    <div>
      ...
      <div style={{background:'#e3f2fd', color:'#333', padding:'8px', marginBottom:'20px', overflow:'hidden', whiteSpace:'nowrap', borderRadius:'4px', border:'1px solid #90caf9'}}>
          <marquee>{attendanceString}</marquee>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', animation: 'fadeIn 0.5s' }}>
          ...
          <div style={cardStyle}>
                ...
                <div style={{display:'flex', justifyContent:'space-around', marginBottom:'15px'}}>
                    <div style={{textAlign:'center'}}>Old: <b>{stats.old_students}</b></div>
                    <div style={{textAlign:'center'}}>New: <b>{stats.new_students}</b></div>
                    <div style={{textAlign:'center'}}>Svr: <b>{stats.servers}</b></div>
                </div>
...
function ParticipantList({ courses, refreshCourses }) {
  ...
  const [swappingSeat, setSwappingSeat] = useState(null); const [newSeatNo, setNewSeatNo] = useState(''); const [printMode, setPrintMode] = useState(''); const [draggedStudent, setDraggedStudent] = useState(null);
  ...
  const SeatBox = ({ p, label }) => (
      <div draggable={!!p} onDragStart={()=>setDraggedStudent(p)} onDragOver={e=>e.preventDefault()} onDrop={()=>handleDrop(label, p)} onClick={() => setSwappingSeat({ p, label })}
           ...
           {p ? <><b style={{color:'blue'}}>{p.dhamma_hall_seat_no}</b><div style={{overflow:'hidden', whiteSpace:'nowrap', width:'90%'}}>{(p.full_name || '').substring(0, 10)}</div><div>{p.conf_no}</div><div style={{fontSize:'8px', color:'#666'}}>P:{p.pagoda_cell_no} D:{p.dining_seat_no}</div></>) : <span style={{color:'#ccc'}}>{label}</span>}
      </div>
  );
