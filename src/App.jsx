...
// --- CORRECTED STUDENT FORM COMPONENT ---
// --- UPDATED STUDENT FORM WITH INTELLIGENT REPRINT ---
function StudentForm({ courses, preSelectedRoom, clearRoom }) {
  ...
  // --- NEW REPRINT LOGIC ---
  const handleReprint = () => {
    // 1. If we just saved, use that data
    if (printData && printData.studentName === selectedStudent?.full_name) {
        setShowReceipt(true);
        setTimeout(() => window.print(), 500);
        return;
    }

    // 2. If no fresh save, but student is selected and "Attending", reconstruct data
    if (selectedStudent && selectedStudent.status === 'Attending') {
        const courseObj = courses.find(c => c.course_id == formData.courseId);
        const reconstructedData = {
            courseName: courseObj?.course_name || 'Course',
            ...
            language: selectedStudent.discourse_language || 'English',
            pagoda: selectedStudent.pagoda_cell_no,
            special: selectedStudent.special_seating
        };
        setPrintData(reconstructedData);
        setShowReceipt(true);
        setTimeout(() => window.print(), 500);
    } else {
        alert("Please select a student who has already checked in to reprint.");
    }
  };
      ...
      {showReceipt && printData && (
          <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.8)', zIndex:9999, display:'flex', justifyContent:'center', alignItems:'center'}}>
              <div style={{background:'white', padding:'20px', borderRadius:'10px', width:'350px'}}>
                  ...
                  <div id="receipt-print-area" style={{padding:'15px', border:'2px solid black', fontFamily:'Helvetica, Arial, sans-serif', color:'black'}}>
                      <div style={{textAlign:'center', fontWeight:'bold', marginBottom:'8px'}}>
                          <div style={{fontSize:'18px', textTransform:'uppercase'}}>Dhamma Nagajjuna 2</div>
                          <div style={{fontSize:'12px'}}>Vipassana International Meditation Center</div>
                      </div>
                      <div style={{borderBottom:'2px solid black', margin:'5px 0'}}></div>
                      <div style={{fontSize:'12px', marginBottom:'10px', display:'flex', justifyContent:'space-between'}}>
                          <span><strong>Course:</strong> {printData.courseName}</span>
                          <span><strong>Dates:</strong> {printData.from} - {printData.to}</span>
                      </div>
                      <div style={{background:'#eee', padding:'5px', fontWeight:'bold', fontSize:'14px', border:'1px solid #ccc', marginBottom:'10px', textAlign:'center'}}>
                          {printData.studentName} ({printData.confNo})
                      </div>
                      <table style={{width:'100%', fontSize:'14px', borderCollapse:'collapse', border:'1px solid black'}}>
                        <tbody>
                          <tr><td style={{border:'1px solid black', padding:'4px'}}>Room</td><td style={{border:'1px solid black', padding:'4px', fontWeight:'bold'}}>{printData.roomNo}</td></tr>
                          <tr><td style={{border:'1px solid black', padding:'4px'}}>Dining Seat</td><td style={{border:'1px solid black', padding:'4px', fontWeight:'bold'}}>{printData.seatNo} ({printData.seatType || 'Chair'})</td></tr>
                          <tr><td style={{border:'1px solid black', padding:'4px'}}>Lockers</td><td style={{border:'1px solid black', padding:'4px', fontWeight:'bold'}}>M:{printData.lockers} / V:{printData.lockers}</td></tr>
                          ...
                          {printData.pagoda && <tr><td style={{border:'1px solid black', padding:'4px'}}>Pagoda</td><td style={{border:'1px solid black', padding:'4px', fontWeight:'bold'}}>{printData.pagoda}</td></tr>}
                        </tbody>
                      </table>
                      <div style={{textAlign:'center', fontSize:'10px', fontStyle:'italic', marginTop:'15px'}}>~ May You Be Happy ~</div>
                  </div>
                  <div className="no-print" style={{marginTop:'20px'}}><button onClick={() => window.print()} style={{width:'100%', padding:'12px', background:'#007bff', color:'white', border:'none', borderRadius:'6px'}}>PRINT RECEIPT</button></div>
              </div>
...
// --- CORRECTED PARTICIPANT LIST (FULL TOOLBAR RESTORED) ---
// --- FULLY RESTORED PARTICIPANT LIST (NO DINING SWAP) ---
function ParticipantList({ courses, refreshCourses }) {
  ...
  const [viewMode, setViewMode] = useState('list'); // 'list', 'dining', 'pagoda', 'seating'
  ...
  const [showAutoAssignModal, setShowAutoAssignModal] = useState(false); // Kept for Dhamma Hall
  const [assignProgress, setAssignProgress] = useState('');
  const [selectedSeat, setSelectedSeat] = useState(null); // For Dhamma Hall moving

  // Seating Config (Dhamma Hall)
  const [seatingConfig, setSeatingConfig] = useState({ mCols: 10, mRows: 8, mChowky: 2, fCols: 7, fRows: 8, fChowky: 2 });
  
  const API_URL = "https://course-manager-backend-cd1m.onrender.com"; 
  
  // Styles
  const toolBtn = (bg) => ({ padding: '8px 16px', border: 'none', borderRadius: '20px', background: bg, color: 'white', cursor: 'pointer', fontSize: '13px', fontWeight:'bold', display:'flex', alignItems:'center', gap:'5px' });
  const btnStyle = (isActive) => ({ padding: '8px 16px', border: '1px solid #ddd', borderRadius: '20px', cursor: 'pointer', background: isActive ? '#007bff' : '#fff', color: isActive ? 'white' : '#555', fontWeight: '600', fontSize:'13px', display:'flex', alignItems:'center', gap:'5px' });
  ...
  const sortedList = React.useMemo(() => { 
      ...
      if (sortConfig.key) { 
          items.sort((a, b) => { 
              ...
              if (valA === null || valA === undefined) valA = ''; if (valB === null || valB === undefined) valB = '';
  ...
  const handleDiningExport = () => { 
    ...
    const headers = ["Seat", "Name", "Gender", "Room"]; 
    const rows = arrived.map(p => [p.dining_seat_no, `"${p.full_name}"`, p.gender, p.room_no]); 
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(","), ...rows.map(e => e.join(","))].join("\n"); 
  ...
  const prepareReceipt = (student) => { 
      ...
      setPrintReceiptData({ 
          ...
          lockers: student.mobile_locker_no || '-', 
  ...
  const prepareBulkTokens = () => { 
      ...
      setPrintBulkData(valid.sort((a,b)=>a.dhamma_hall_seat_no.localeCompare(b.dhamma_hall_seat_no, undefined, {numeric:true})).map(student=>({ seat: student.dhamma_hall_seat_no, name: student.full_name, conf: student.conf_no }))); 
      setTimeout(()=>window.print(), 500); 
  };
          ...
          <div style={{background:'white', padding:'25px', borderRadius:'12px', marginBottom:'20px'}}>
              ...
              <div className="print-area" id="print-summary" style={{padding:'20px'}}>
                  ...
                  <table style={{width:'100%', borderCollapse:'collapse', border:'1px solid black', marginBottom:'20px'}}>
                      ...
                      <tbody>
                          <tr><td style={tdPrint}>MALE</td><td style={tdPrint}>{getCount('m', 'OLD')}</td><td style={tdPrint}>{getCount('m', 'NEW')}</td><td style={tdPrint}>0</td><td style={tdPrint}>0</td><td style={tdPrint}><strong>{getCount('m', 'OLD') + getCount('m', 'NEW')}</strong></td></tr>
                          <tr><td style={tdPrint}>FEMALE</td><td style={tdPrint}>{getCount('f', 'OLD')}</td><td style={tdPrint}>{getCount('f', 'NEW')}</td><td style={tdPrint}>0</td><td style={tdPrint}>0</td><td style={tdPrint}><strong>{getCount('f', 'OLD') + getCount('f', 'NEW')}</strong></td></tr>
                          <tr style={{background:'#f0f0f0', fontWeight:'bold'}}><td style={tdPrint}>TOTAL</td><td style={tdPrint}>{getCount('m', 'OLD') + getCount('f', 'OLD')}</td><td style={tdPrint}>{getCount('m', 'NEW') + getCount('f', 'NEW')}</td><td style={tdPrint}>0</td><td style={tdPrint}>0</td><td style={tdPrint}>{arrived.length}</td></tr>
                      </tbody>
                  </table>
              </div>
          </div>
  ...
  // 2. DINING VIEW (No Swap, Just Lists)
  if (viewMode === 'dining') { 
      ...
      const renderTable = (list, title, color, sectionId) => ( 
          <div id={sectionId} style={{marginBottom:'40px', padding:'20px', border:`1px solid ${color}`}}> 
             <div className="no-print" style={{textAlign:'right', marginBottom:'10px'}}><button onClick={handleDiningExport} style={toolBtn('#17a2b8')}>CSV</button> <button onClick={() => {const style=document.createElement('style'); style.innerHTML=`@media print{body *{visibility:hidden}#${sectionId},#${sectionId} *{visibility:visible}#${sectionId}{position:absolute;left:0;top:0;width:100%}}`; document.head.appendChild(style); window.print(); document.head.removeChild(style);}} style={{...toolBtn(color), marginLeft:'10px'}}>Print {title}</button></div> 
             <h2 style={{color:color, textAlign:'center'}}>{title} Dining Plan</h2> 
             <table style={{width:'100%', borderCollapse:'collapse', border:'1px solid #000'}}><thead><tr><th style={thPrint}>S.N.</th><th style={thPrint}>Seat</th><th style={thPrint}>Name</th><th style={thPrint}>Cat</th><th style={thPrint}>Room</th></tr></thead>
             <tbody>{list.map((p,i)=>(<tr key={p.participant_id}><td style={tdPrint}>{i+1}</td><td style={{...tdPrint, fontWeight:'bold', fontSize:'14px'}}>{p.dining_seat_no}</td><td style={tdPrint}>{p.full_name}</td><td style={tdPrint}>{getCategory(p.conf_no)}</td><td style={tdPrint}>{p.room_no}</td></tr>))}</tbody></table> 
          </div> 
      ); 
      return ( 
          <div style={{background:'white', padding:'25px', borderRadius:'12px'}}> 
             ...
             {renderTable(arrived.filter(p=>(p.gender||'').toLowerCase().startsWith('m')).sort(sorter), "MALE", "#007bff", "pd-m")} 
             {renderTable(arrived.filter(p=>(p.gender||'').toLowerCase().startsWith('f')).sort(sorter), "FEMALE", "#e91e63", "pd-f")} 
          </div> 
      ); 
  }
  ...
  if (viewMode === 'seating') {
      ...
      const Box = ({p, l}) => {
          let sVal = '-', lVal = '-';
          if (p && p.courses_info) {
             const sMatch = p.courses_info.match(/S\s*[:=-]?\s*(\d+)/i); const lMatch = p.courses_info.match(/L\s*[:=-]?\s*(\d+)/i);
             if(sMatch) sVal = sMatch[1]; if(lMatch) lVal = lMatch[1];
          }
          return (
              <div onClick={() => handleSeatClick(l, p)} style={{border: '2px solid black', background:'white', height:'100%', fontSize:'10px', display:'flex', flexDirection:'column', cursor:'pointer', position:'relative', minHeight:'90px'}}>
                  <div style={{display:'flex', justifyContent:'space-between', borderBottom:'1px solid black', padding:'2px 4px', fontWeight:'bold', fontSize:'13px', background:'#fff'}}><span>{l}</span><span>{p ? (p.room_no || '') : ''}</span></div>
                  <div style={{flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'2px', textAlign:'center', fontWeight:'bold', fontSize:'11px', lineHeight:'1.2'}}>{p ? p.full_name : ''}</div>
                  {p && (<div style={{borderTop:'1px solid black', padding:'2px 4px', fontSize:'10px', display:'flex', justifyContent:'space-between', background:'#f9f9f9'}}><span>({getCategory(p.conf_no).charAt(0)})</span><span>s:{sVal} L:{lVal}</span><span>{p.age}</span></div>)}
              </div>
          );
      };
      
      const renderGrid = (map, cols, rows) => {
          let g=[]; for(let r=rows; r>=1; r--) { let cells=[]; cols.forEach(c => cells.push(<Box key={c+r} l={c+r} p={map[c+r]} />)); g.push(<div key={r} style={{display:'grid', gridTemplateColumns:`repeat(${cols.length}, 130px)`, gridAutoRows:'95px', gap:'-1px'}}>{cells}</div>); } return g;
      };
  ...
  return (
    <div style={{background:'white', padding:'25px', borderRadius:'12px', boxShadow:'0 4px 6px rgba(0,0,0,0.05)', marginBottom:'20px'}}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:'20px', alignItems:'center'}}>
         <div style={{display:'flex', gap:'10px'}}><select style={{padding:'10px', borderRadius:'6px', border:'1px solid #ccc'}} onChange={e=>setCourseId(e.target.value)}><option value="">-- Select Course --</option>{courses.map(c=><option key={c.course_id} value={c.course_id}>{c.course_name}</option>)}</select><input style={{padding:'10px', borderRadius:'6px', border:'1px solid #ccc'}} placeholder="Search..." onChange={e=>setSearch(e.target.value)} disabled={!courseId} /></div>
      ...
      {courseId && <div style={{marginBottom:'15px', padding:'15px', background:'#fff0f0', border:'1px solid red', borderRadius:'5px', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
          <span style={{fontWeight:'bold', color:'red'}}>⚠️ Admin Zone:</span>
          <div>
      ...
      <div style={{overflowX:'auto', border:'1px solid #eee', borderRadius:'8px'}}>
        <table style={{width:'100%', fontSize:'13px', borderCollapse:'collapse'}}>
          <thead><tr style={{background:'#f8f9fa', textAlign:'left'}}>{['S.N.', 'FULL_NAME','CONF_NO', 'ROOM_NO','DINING_SEAT_NO', 'STATUS'].map(k=><th key={k} style={{padding:'12px', borderBottom:'2px solid #ddd', cursor:'pointer'}} onClick={()=>handleSort(k.toLowerCase())}>{k}</th>)}<th>PRINTS</th><th>ACTIONS</th></tr></thead>
          <tbody>
            {sortedList.map((p, i) => (
              <tr key={p.participant_id} style={{borderBottom:'1px solid #eee', background: p.status==='Attending'?'white':'#fff5f5'}}>
                <td style={{padding:'12px', color:'#777'}}>{i+1}</td>
                ...
                <td style={{padding:'12px', fontWeight:'bold', color: p.status==='Attending'?'green':'orange'}}>{p.status}</td>
                <td style={{padding:'12px'}}>
                   <button onClick={() => prepareReceipt(p)} style={{marginRight:'5px', padding:'4px 8px', background:'#e3f2fd', border:'1px solid #90caf9', cursor:'pointer', fontSize:'11px'}}>Receipt</button>
                   <button onClick={() => prepareToken(p)} style={{padding:'4px 8px', background:'#fff3cd', border:'1px solid #ffeeba', cursor:'pointer', fontSize:'11px'}}>Token</button>
                </td>
                <td style={{padding:'12px'}}>
                   <button onClick={() => setEditingStudent(p)} style={{padding:'5px', cursor:'pointer'}}>✏️</button>
                   <button onClick={() => handleDelete(p.participant_id)} style={{padding:'5px', color:'red', cursor:'pointer'}}>🗑️</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {/* PRINT MODALS & BULK TOKENS */}
      {printReceiptData && <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:9999, display:'flex', justifyContent:'center', alignItems:'center'}}><div style={{background:'white', padding:'20px', width:'300px'}}><div id="receipt-print-area" style={{border:'2px solid black', padding:'10px'}}><h3>{printReceiptData.courseName}</h3><p>{printReceiptData.studentName}</p></div><button onClick={()=>window.print()} style={{width:'100%', marginTop:'10px'}}>Print</button><button onClick={()=>setPrintReceiptData(null)} style={{width:'100%', marginTop:'5px'}}>Close</button></div><style>{`@media print { body * { visibility: hidden; } #receipt-print-area, #receipt-print-area * { visibility: visible; } #receipt-print-area { position: absolute; left: 0; top: 0; width: 100%; } }`}</style></div>}
      
      {printBulkData && (
          <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.8)', zIndex:9999, display:'flex', justifyContent:'center', alignItems:'center'}}>
              <div style={{background:'white', padding:'20px', borderRadius:'5px', width:'400px', maxHeight:'80vh', overflowY:'auto'}}>
                  <button onClick={() => setPrintBulkData(null)} style={{float:'right', background:'red', color:'white', border:'none', borderRadius:'50%', width:'25px', height:'25px'}}>X</button>
                  <h3 style={{textAlign:'center'}}>🖨️ Bulk Tokens</h3>
                  <button onClick={() => window.print()} style={{width:'100%', padding:'10px', background:'#007bff', color:'white', marginBottom:'20px'}}>PRINT ALL</button>
                  <div id="bulk-token-print-area">
                      {printBulkData.map((t, i) => (
                          <div key={i} className="token-slip" style={{border:'2px dashed black', padding:'15px', marginBottom:'10px', textAlign:'center', pageBreakInside: 'avoid', pageBreakAfter: 'always'}}>
                              <div style={{fontSize:'12px', textTransform:'uppercase'}}>Dhamma Seat</div>
                              <div style={{fontSize:'40px', fontWeight:'900', margin:'5px 0'}}>{t.seat}</div>
                              <div style={{fontSize:'16px', fontWeight:'bold'}}>{t.name}</div>
                              <div style={{fontSize:'12px'}}>{t.conf}</div>
                          </div>
                      ))}
                  </div>
              </div>
              <style>{`@media print { @page { size: auto; margin: 0; } body * { visibility: hidden; } #bulk-token-print-area, #bulk-token-print-area * { visibility: visible; } #bulk-token-print-area { position: absolute; left: 0; top: 0; width: 100%; } .token-slip { page-break-after: always; display: block; border-bottom: 2px dashed black !important; } }`}</style>
          </div>
      )}

      {/* EDIT MODAL */}
      {editingStudent && (<div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', justifyContent:'center', alignItems:'center', zIndex:1000}}><div style={{background:'white', padding:'30px', width:'600px', borderRadius:'10px'}}><h3>Edit Student</h3><form onSubmit={handleEditSave}><div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'15px'}}><div><label>Full Name</label><input value={editingStudent.full_name} onChange={e=>setEditingStudent({...editingStudent, full_name:e.target.value})} style={{width:'100%', padding:'10px'}} /></div><div><label>Conf No</label><input value={editingStudent.conf_no||''} onChange={e=>setEditingStudent({...editingStudent, conf_no:e.target.value})} style={{width:'100%', padding:'10px'}} /></div></div><div style={{textAlign:'right'}}><button onClick={()=>setEditingStudent(null)} style={{marginRight:'10px', padding:'8px', cursor:'pointer'}}>Cancel</button><button type="submit" style={{padding:'8px 16px', background:'#007bff', color:'white', border:'none', borderRadius:'5px', cursor:'pointer'}}>Save</button></div></form></div></div>)}
    </div>
  );
}
