...
// --- 2. GLOBAL ACCOMMODATION MANAGER (Strict Room Validation + Old/New Stats) ---
        ...
        {isOccupied ? (
          <div style={{fontSize:'11px', color: isArrived ? '#c62828' : '#ef6c00', marginTop:'4px'}}>
            ...
            <div>
                ...
                <button onClick={(e)=>{e.stopPropagation(); onRoomClick(room.room_no)}} style={{marginTop:'5px', fontSize:'10px', background:'#4caf50', color:'white', border:'none', borderRadius:'3px', cursor:'pointer', width:'100%'}}>Assign</button>
            </div>
        )}
    ...
    <div style={cardStyle}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
         <div style={{border:'1px solid #90caf9', borderRadius:'8px', padding:'10px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'8px'}}>{rooms.filter(r=>r.gender_type==='Male').map(renderRoom)}</div>
         </div>
         <div style={{border:'1px solid #f48fb1', borderRadius:'8px', padding:'10px'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:'8px'}}>{rooms.filter(r=>r.gender_type==='Female').map(renderRoom)}</div>
         </div>
      </div>
...
// --- 3. ONBOARDING FORM (Strict Room Dropdown) ---
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:'10px'}}>
          ...
          <div>
            ...
            <select style={{...inputStyle, background: preSelectedRoom ? '#e8f5e9' : 'white'}} value={formData.roomNo} onChange={e => setFormData({...formData, roomNo: e.target.value})} required>
              ...
              {rooms.map(r => <option key={r.room_id} value={r.room_no}>{r.room_no}</option>)}
            </select>
          </div>
